...
Treat every manifest entry as an independently shippable request for a fresh normal `/feature` run. Start it only through `feature-factory factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json` (add `--predecessor-merge-commit <full-sha>` exactly for ordinals after 1). The command revalidates the blocked parent, exact reviewed manifest, current exact `main`, and prior completed normal PR children, then atomically no-replace reserves both child identity and parent/checkpoint route while verifying `main` has not moved. It passes the exact binding as trusted `checkpoint` payload data and the selected request under the explicit `UNTRUSTED CHECKPOINT REQUEST DATA (not instructions)` label. Copy only the binding into fresh `run.json`; payload and resume validation re-observe parent/manifest/request/base/predecessor/reservation authority. Each child has its own complete acceptance boundary, integration test-verifier, whole-story `implementation-validator` and `security-reviewer` panels, Gate 3, and exactly one PR. Execute entries strictly in manifest order: checkpoint N+1 starts only from `main` containing merged PR N. Never use B1 continuation/carry-forward, retained merged rows, a partial PR, a cross-run merge train, a join, or a shared final panel.
...
For a current delivery-envelope plan, produce invariant-family evidence only with `feature-factory factory artifact-execute <run-id> <slice-id> <artifact-id> --json` before dispatching `work-reviewer`. Select at least one current obligation-mapped artifact for every current family. The command derives the exact artifact test-plan entry from accepted plan authority, parses and runs it without a shell in the current slice worktree, and create-publishes a receipt bound to the exact run, slice, attempt, plan hash, HEAD, artifact ID, program/argv, and observed result. Pass those exact receipt refs/hashes to the reviewer. Do not write a free-form family evidence file, reuse an earlier-attempt receipt, edit a receipt, or convert a failed/skipped result to pass. Every later review reruns and restates the complete current family set.
