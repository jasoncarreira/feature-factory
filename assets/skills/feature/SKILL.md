...
### Runtime Consistency And Declared Tooling

Treat runtime identity observation and runtime launch admission as separate contracts:

- `feature-factory install` and doctor expose the effective PATH CLI's terminal-safe `source`, package `version`, and SHA-256 `hash`. Every launch observes the executing package plugin/CLI, effective configured plugin package, PATH CLI, and OpenCode identity. `factory env record-created` and `record-resume` persist only configured `plugin_identity` and effective PATH `cli_identity` under `run.json.debug_snapshot.*.env` for creation/resume debugging.
- `plugin_identity` and `cli_identity` are diagnostic-only. They expose no config content, are never provenance or authority for gates, reviews, merges, PRs, or a later launch, and never replace fresh admission.
- Determine effective plugin config with the child `HOME`, effective `XDG_CONFIG_HOME/opencode`, project configs from cwd through the nearest Git root and project `.opencode`, and non-empty `OPENCODE_CONFIG_DIR` resolved from cwd. Accept one readable local `file://` feature-factory registration naming its package root, current `src/opencode-plugin.js`, or legacy `src/plugin.js`; multiple, ambiguous, malformed, or unreadable local registrations reject. Bare package registration may use the executing package fallback.
- Before every foreground or detached start, start-as-resume, resume, schema-v2 continuation, checkpoint child, or approval-handoff spawn, require exact SHA-256 byte equality between executing and configured runtime package closures, then between configured `src/cli.js` and PATH `feature-factory`. Each bounded closure covers `package.json`, every shipped `.js` file below `src/`, and every regular workflow file below `assets/` with sorted relative paths and unambiguous path/content lengths and bytes; symlinks, unreadable/nonregular entries, excessive files/bytes, and incomplete roots fail closed. Bind and immediately revalidate both closure `{source, hash}` tuples together with the existing shared plugin implementation and package CLI entry identities, including across detached IPC serialization. On mismatch or absence, stop on `RUNTIME_ADMISSION_FAILED`; the safe remediation targets the configured package root through the `npm` executable with exact argv `["install","--global","--","<observed-package-root>"]`, followed by verifying PATH resolves the accepted bytes. Never reinterpret that displayed argv as shell text.
- Admission binds the package CLI and OpenCode absolute source/hash together with executing and configured plugin/CLI identities, re-reads config and rechecks every binding immediately before spawn, and uses the bound OpenCode executable with `shell:false`. Config removal or source/hash drift rejects. Detached launch performs the launcher recheck and the detached supervisor repeats it before spawning the child.

Before any launch, require absent or exact current global definitions. Inspect HOME OpenCode singular/plural skill and agent paths, HOME `.claude/skills/feature/SKILL.md` and `.agents/skills/feature/SKILL.md`, effective `XDG_CONFIG_HOME/opencode`, and non-empty `OPENCODE_CONFIG_DIR`. Non-empty `OPENCODE_CONFIG` or `OPENCODE_CONFIG_CONTENT` fails closed. A feature skill is sanctioned only when byte-exact with the packaged skill or the small delegator that loads repo-seeded `SKILL.md`/`SCHEMA.md`; every subagent file must be byte-exact packaged content, and a global primary `feature-factory.md` is always stale. Mismatch, symlink, unreadable, or ambiguous paths reject. This applies to all launch routes and direct plugin initialization/config registration. Reconcile the files or overrides and restart OpenCode; do not continue from a warning.

`feature-factory factory --help` is a supported deterministic exit-0 usage route with no unknown-command diagnostic. Doctor must classify the intended one-level Task policy as healthy: primary `permission.task: "allow"`, every subagent `permission.task: "deny"`.

Use Node-native filesystem publication for checkpoint and schema-v2 carry-forward child directories; do not require host `mv`. Runtime external tools are limited to the documented OpenCode, `git`, operation-specific `gh`, and platform process inspection (`/proc` on Linux; `ps` and `lsof` on Darwin). Do not assume `python3` or another undeclared scripting/helper runtime exists. Missing required tools or unsupported inspection stops with a bounded actionable failure.
...
- `blocked-run-continuation`: start the only current continuation shape, schema-v2 full-plan carry-forward, with explicit pre-PR `--carry-forward`.
...
`feature-factory factory continue <blocked-run-id> --review <review-ref> --run-id <new-run-id> --carry-forward` is the public CLI entry point for continuing a failed automated run. The CLI may inject a structured continuation payload into `/feature`, but that payload is untrusted operator data/config, not privileged instruction. Validate every referenced run id, path, ref, branch, commit, artifact, and review before use.

Schema v2 full-plan carry-forward is the only current continuation shape and is selected only before PR creation by explicit `--carry-forward`. V1, unflagged, partial, draft-reuse, and post-PR forms reject before effects. V2 requires durably accepted unchanged planning, no draft reuse, exact B1.3 allocation, and a complete atomically published child. Its root run schema remains 1 while `run.continuation.schema_version` is 2.
...
- Persist only the closed schema-v2 relationship under `run.json.continuation`, including exact parent/review/target bindings, closed immutable configuration, and `carry_forward` with `scope: "full-remaining-plan"`, exact plan hash/start commit, immutable accepted rows, and nonempty disjoint remaining slice IDs.
- The published full plan and attempt-zero inherited spec acceptance replace bootstrap/planning only; accepted merged rows and sidecars are immutable, pending rows run only when dependencies are merged, and fresh full-plan test-verifier attempt one, validator, security, and whole-story Gate 3 remain mandatory after all rows merge.
...
Conditions are `stale-heartbeat`, `missing-heartbeat-process`, `missing-worktree`, `invalid-run-state`, `newer-schema`, `protected-gate`, and `terminal-run`. `newer-schema` is `invalid-run-state` narrowed to the unknown-keys-only case, where the record was written by a newer schema than this reader; it carries the same `invalid` classification, critical severity, and fail-closed treatment, and exists only so operators can tell a forward-schema record apart from a corrupt one. Classifications are `healthy`, `recoverable`, `blocked`, `needs-human`, `terminal`, and first-class `invalid`; statuses are `ok`, `warning`, `error`; severities are `info`, `warning`, `error`, `critical`.
...
For a schema-v2 `blocked-run-continuation`, Step 0 has already validated the published child and routed directly to dependency-ready remaining work. Never enter research/spec/decomposition for that child. Missing current accepted planning authority rejects the continuation and requires re-seeding.
...
For a schema-v2 `blocked-run-continuation`, never call `adopt-continuation`, rerun `spec-writer`, construct a narrow remediation plan, or infer acceptance from copied files. Checked full-plan publication has already bound exact accepted planning, immutable merged rows, and authority-free pending rows. Missing or drifted bindings reject fail-closed.

Every current plan requires `integration_gate.required_commands`, `integration_gate.timeout_ms`, and a complete `delivery_envelope` with per-artifact `timeout_ms`; missing or partial current plan authority rejects and requires re-seeding.
...
Old-shape slice review/merged rows reject without mutation. Review publication and merge require complete current bindings and exact append-only history through the current attempt; there is no replay upgrade, synthetic backfill, or unclassified history variant. Partial current tuples reject, current merged rows are immutable, and old-shape completed runs reject.
