...
After decomposition review, invoke the same `feature-factory factory slices-seed <run-id> --from plan/slices.json --json` path before recording accepted `work-decomposer` state. An `admit` result follows the ordinary slice-seeding and acceptance flow unchanged. A `checkpoint` result is terminal for the oversized parent: the command content-addresses and persists the deterministic checkpoint manifest, writes the closed blocked reason `oversized-plan-checkpoint-routing-required`, and leaves `run.json.slices` unseeded. Stop immediately before accepted decomposer state, Gate 2, slice branch/worktree/dispatch, implementation, test-verifier, panels, Gate 3, or PR creation. Do not invoke a generic terminal command after this checked route and do not use the older `REDESIGN-REQUIRED` branch.

Checkpoint terminalization uses the ordinary lock-protected terminal observation. The first tokenless `slices-seed` call succeeds normally for `admit`; for an exact `checkpoint` decision it rejects unchanged with the missing terminal-observation error. Stop any decomposition heartbeat and drain/ack steering, open `feature-factory factory boundary-open <run-id> terminal --json`, then retry the exact same plan bytes with `feature-factory factory slices-seed <run-id> --from plan/slices.json --boundary-token <terminal-boundary.token> --json`. Never open this terminal boundary speculatively before an admitted seed. The retry rejects pending or consumed-unacknowledged steering, active heartbeat, stale/missing/wrong boundaries, action claims, pre-PR fences, Gate 2 state, and non-placeholder test-verifier state before publishing an artifact. It consumes the exact generation/state-hash-bound boundary only with the blocked routing result and reobserves that authority immediately before replacing `run.json`.

Treat every manifest entry as an independently shippable request for a fresh normal `/feature` run. Each request has its own complete acceptance boundary, integration test-verifier, whole-story `implementation-validator` and `security-reviewer` panels, Gate 3, and exactly one PR. Execute entries strictly in manifest order: checkpoint N+1 may start only from `main` containing merged PR N. Never split this oversized parent with B1 continuation/carry-forward, retained merged rows, a partial PR, a cross-run merge train, a join, or a shared final panel. The parent manifest and its terminal result are routing evidence only; no model-authored plan file, prose, boolean, or claim may replace them.
