...
Treat every manifest entry as an independently shippable request for a fresh normal `/feature` run. Start it only through `feature-factory factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json` (add `--predecessor-merge-commit <full-sha>` exactly for ordinals after 1). The command revalidates the blocked parent and reviewed manifest, validates canonical GitHub origin identity, fetches exact remote `refs/heads/main` into a checked temporary ref, and repeats that observation immediately before launch without trusting local or remote-tracking main. Retry only the exact same command: a pre-launch `reserved` claim remains recoverable, but an existing child is adopted only when a matching `launching` or `launched` reservation already predates it; never create a reservation around existing child state. Every predecessor must expose the same exact claim through both reservation refs, bind exactly its `run.checkpoint`, be exactly `launched`, prove the full pipeline, and have its canonical GitHub merge commit on freshly fetched remote main. Local-only, stale, forged, non-launched, wrong-repository, raced, or divergent authority is never sufficient.
