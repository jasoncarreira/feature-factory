...
Treat every manifest entry as an independently shippable request for a fresh normal `/feature` run. Start it only through `feature-factory factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json` (add `--predecessor-merge-commit <full-sha>` exactly for ordinals after 1). Parent/default `main` and `refs/heads/main` normalize to canonical remote `refs/heads/main`; other refs reject. The command fetches that ref, atomically reserves child/route/branch, creates the exact-base worktree, and re-fetches before launch. Retry only the exact same command. On resume, freshly reobserve remote main: fail a reserved/launching stale child before implementation, preserve launched run identity across descendant advances, and stop for explicit recovery on divergence—never reset or rebase the child branch. Existing-child and predecessor reservation rules remain exact and fail closed.
