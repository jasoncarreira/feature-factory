...
- Bootstrap a fresh child run using `--run-id <new-run-id>` and persist the relationship under `run.json.continuation` with the parent run id, parent branch/commit/worktree refs, `review_ref`, source artifact/evidence/review refs, created timestamp, and an operator-supplied summary when present.
