...
- `feature-factory install` and doctor expose the effective PATH CLI's terminal-safe `source`, package `version`, and SHA-256 `hash`. Every launch observes package/plugin CLI, PATH CLI, and OpenCode identity. `factory env record-created` and `record-resume` persist only the effective PATH CLI observation at `run.json.debug_snapshot.*.env.cli_identity` for creation/resume debugging.
- `cli_identity` is diagnostic-only. It is never provenance or authority for gates, reviews, merges, PRs, or a later launch, and it never replaces fresh admission.
- Before every foreground or detached start, start-as-resume, resume, schema-v2 continuation, checkpoint child, or approval-handoff spawn, require exact SHA-256 byte equality between package `src/cli.js` and PATH `feature-factory`. On mismatch or absence, stop on `RUNTIME_ADMISSION_FAILED`; the safe remediation is the `npm` executable with exact argv `["install","--global","--","<observed-package-root>"]`, followed by verifying PATH resolves the accepted bytes. Never reinterpret that displayed argv as shell text.
- Admission binds package CLI and effective OpenCode absolute source/hash, rechecks both immediately before spawn, and uses the bound OpenCode executable with `shell:false`. Detached launch performs the launcher recheck and the detached supervisor repeats it before spawning the child.
