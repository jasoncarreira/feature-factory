...
Treat every manifest entry as an independently shippable request for a fresh normal `/feature` run. Start it only through `feature-factory factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json` (add `--predecessor-merge-commit <full-sha>` exactly for ordinals after 1). The command revalidates the blocked parent, exact reviewed manifest, and current main, then atomically advances both child/route refs through reservation states. Retry only the exact same command: a crash before launch remains recoverable, an exact existing child is adopted, and launching/unknown or conflicting authority fails closed without another launch. Every predecessor must prove all slices merged, fresh accepted passing checked tests, exact-head passing panels, Gate 3, and the exact completed PR; bounded canonical GitHub observation must confirm that PR merged at the supplied/observed merge commit on current main. Local-only or divergent merge claims are never authority. Payload and resume re-observe the exact binding and reservation.
