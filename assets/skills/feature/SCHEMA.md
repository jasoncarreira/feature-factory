...
B3.4 keeps remediation feasibility separate from successful finish-and-disclose publication. Sibling-owned and contract-change fixes from a `REJECT` still route to the owner, merged-slice repair, or plan/brief amendment under the existing rules. Checked builder context identifies exact `declared_paths`, current reviewed `effective_paths`, and forecast paths, but forecasts never grant mutation or merge authority. For every Git-observed concrete path outside declared ownership, slice evidence contains sorted unique `ownership_disclosure: [{path,rationale}]` whose path set exactly equals the observed unexpected set and whose rationale is nonempty trimmed NFC-normalized text. New reviews use pathless `ownership_ratification: {schema_version:2,kind:"factory-derived-modified-extension"}`; the factory, never the reviewer, derives eligible paths and retains each exact evidence rationale. A v2 `APPROVE` may admit only a newly added zero-owner private regular file or a content-only modification of an unchanged-mode private regular file with zero owners or one current accepted non-touching sibling owner. Delete, rename/copy endpoint, mode/type change, symlink, submodule, generated/privileged, ambiguous, touching, missing, stale, partial, or cross-bound authority denies publication. The shared fail-closed privileged/control-plane policy remains unchanged and requires explicit declared plan ownership.
...
The object is closed to exactly `scope`, `plan_ref`, `plan_hash`, `start_commit`, `accepted_slices`, and `remaining_slice_ids`, with `scope: "full-remaining-plan"`. Each `accepted_slices` entry is closed to exactly `id`, `declared_paths`, `effective_paths`, `attempts`, `attempt_reviews`, `evidence_ref`, `evidence_hash`, `review_ref`, `review_hash`, `reviewed_commit`, and `merge_commit`, plus the existing optional `integration_conflict`; ownership arrays and mixed immutable v1/v2 history are copied byte-for-byte through normalization. V2 rationales and complete sibling provenance are retained, and child publication copies every referenced evidence, review, ordinary dispatch claim/closure, invariant-family receipt/claim, and integration-conflict sidecar at its exact hash. A modifying accepted row containing `authority:"non-conflicting-sibling"` is admissible only when `accepted_slices` also contains that exact owner as a merged row and the recorded owner attempt, evidence, review, dispatch, reviewed commit, and diff baseline equal the owner's current accepted binding. A missing owner, an owner still only in `review`, or stale/cross-bound owner provenance rejects before child publication. `remaining_slice_ids` contains IDs only. Accepted rows and remaining IDs stay in PLAN order, are unique and disjoint, and exactly partition the bound plan; they need not form a prefix/suffix split. Remaining child rows seed declared/effective paths from the plan and inherit no attempt authority. The deliberate owner/modifier effective-path overlap remains visible, so unchanged sole-owner consumers continue to treat it as ambiguous; carry-forward creates no general overlap exception.
