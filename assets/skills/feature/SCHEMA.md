...
The supported seam is the public OpenCode **1.18.3** plugin API. `command.execute.before` validates the encoded factory command envelope, but its run id remains an untrusted candidate. Binding requires the exact nonce-bearing live launch claim or a checked durable builder/amendment dispatch; a session lifecycle event alone never binds launch identity. Invalid and unencoded commands bind nothing. `event({event})` exposes session lifecycle information: `session.created`, `session.updated`, and `session.deleted` carry `properties.info.id` plus optional `parentID`, while `session.status`, `session.idle`, and `session.compacted` carry `properties.sessionID`. Observed children inherit only verified bounded run identity from their parent. `tool.execute.before/after` exposes Task `sessionID` plus `callID`; the ordered pair identifies one hook invocation. Session parentage is best-effort from those fields. Prompts, messages, arguments, results, reviews, evidence, and model output are never parsed for correlation identity or exported.
...
- `run.json.debug_snapshot` and `run.json.cost_attribution` are diagnostic records. They do not authorize semantic workflow decisions.
...
The initial child keeps root `schema_version: 1` and continuation `schema_version: 2`; persists closed mode/account/PR/post-PR configuration, limits and retries of 3, and an absent root `review_tier` for null or the exact non-null checkpoint tier. Ordinary B1 copies exact accepted planning/spec review into an attempt-zero accepted `spec-writer` row. Checkpoint-bound B1 preserves only the checkpoint child's ordinary accepted `work-decomposer` row and exact plan/disposition files: it does not create a `spec-writer` row, technical brief, spec verdict, or spec review. Both variants publish an authority-free `{ "agent": "test-verifier", "status": "blocked", "attempts": 0 }` placeholder, copy exact accepted sidecars, and publish PLAN-ordered immutable merged plus attempt-zero pending rows. The child has fresh steering, `gates: {}`, null validator/security, and no inherited test, panel, PR, cost, process, or outcome authority. Fresh execution skips bootstrap/story/research/spec/decomposition, executes dependency-ready pending rows, and advances that test-verifier placeholder to a fresh full-plan attempt one only after all rows merge, followed by fresh validator and security reviewers and whole-story pre-PR.
