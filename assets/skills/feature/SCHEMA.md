...
Fresh checkpoint children persist closed `run.checkpoint` `{schema_version:1,kind:"delivery-checkpoint-child",parent_run_id,parent_run_ref,parent_run_hash,manifest_ref,manifest_hash,checkpoint_id,checkpoint_ordinal,child_run_id,base_ref,base_commit,predecessor_checkpoint_id,predecessor_child_run_id,predecessor_merge_commit}`. Both checkpoint refs point to one canonical `delivery-checkpoint-child-reservation` blob with immutable nonce/binding and state-specific timestamps: `reserved`, `launching`, `launched`, or `unknown` with reason `launch-outcome-indeterminate`. State changes compare-and-swap both refs while verifying exact main. A crash before launch leaves `reserved` and permits exact retry; an exact existing child is adopted; `launching` without a child and `unknown` require reconciliation; stale/concurrent/conflicting transitions reject without another launch. For ordinal N every predecessor must have all slices merged, fresh accepted passing checked test authority, exact-head passing validator/security bindings, approved Gate 3, and a completed exact PR identity. Bounded canonical GitHub observation must report that exact PR merged and expose the exact merge commit; it must contain the child head and be an ancestor of current main, and the immediate commit must equal `--predecessor-merge-commit`. Local-only, fake, stale, open/absent, or divergent observations reject.
