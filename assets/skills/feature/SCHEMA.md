...
The `test-verifier` step is the post-merge integration gate. A transition to `running` requires every durable slice to be `merged`, a positive `attempts` value, and `attempts <= run.max_retries` (default 3). Each gate pass runs the accepted brief's canonical repository-wide command and writes `evidence/test-verifier.attempt-N.json`. Red evidence records the step `rejected` and routes one bounded owner-specific remediation before the complete gate reruns at `N + 1`; max-attempt failure records the step and run blocked. Green evidence is reviewed at `reviews/test-verifier.attempt-N.json` before acceptance. Integration remediation has no separate free-form review subject or uncounted attempt loop.
