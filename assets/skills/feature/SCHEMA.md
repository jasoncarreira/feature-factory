...
OpenCode compaction exact-matches the child session's original injected prompt. Post-compaction model calls receive freshly revalidated checked context through plugin-owned system text; the model-authored compaction summary is non-authoritative. A trusted operator may resolve a callback that returned without closure only with `factory slice-dispatch-reconcile <run-id> <slice-id> <attempt> --authorize-callback-returned-without-closure --json`. The command derives all authority, requires a new clean descendant HEAD, and create-publishes `checked-slice-builder-dispatch-reconciliation` at the same exclusive `.closed.json` ref without a completion token. The slot permits exactly one completion kind: normal callback closure or operator reconciliation. Both bind `dispatch_closure_ref`, `dispatch_closure_hash`, and `completion_head`; all downstream evidence, review, ownership, and HEAD checks remain unchanged.
