...
The active B5.4 delivery contract supersedes the preceding B4 inventory count: the current canonical-source manifest has 196 variants, 195 production-covered rows plus the sole future row `final-plan-descriptor`. B2 added `terminal-result-blocked-nonconvergence`, independently cataloged ordinary and nonconvergent blocked slices, and extended slice rows with attempt-history and checked-dispatch authority. B4 separately catalogs the delivery envelope, invariant-family ledger, reviewed checkpoint plan, typed valid probe, child disposition, routing artifact, terminal route, creation-only publication claim, immutable source, all five progress variants, merged completion, content-addressed final closure, five verification-artifact claim states, and passing/failing verification-artifact receipts. B5.4 production-catalogs all 16 amendment manifests, 12 execution claims, 12 execution receipts, two semantic reviews, four owner-builder dispatch rows, and the two amendment-review dispatch rows in class 9. Every generated mutation reaches its shared validator, checked consistency reader, checked transition, exact claim/receipt consumer, or downstream authority revalidation seam.
...
| Generic integration amendment (implemented B5 class-9 migration) | exactly 48 implemented rows: 16 explicit manifest variants; 12 report/verify claim variants; 12 report/verify receipt variants; two review variants; four builder-dispatch variants; amendment-review dispatch claim and closure | Fixed pre-manifest report tombstone; factory-owned baseline/staged failing-to-passing reproduction; exact owner and pristine-pending consumer snapshot; append-only attempts; fresh reviewed commit/tree and Git-observed paths; immutable checked reviewer callback provenance; staged integration; old-OID feature publication; disjoint legacy/generic cutover with all eight historical legacy rows retained |

The B5.1 design registered the generic integration amendment as the `planned B5 successor within class 9` with exactly 48 planned rows; B5.4 activates all 48 rows and narrows only new legacy report admission. This historical label does not make the implemented route optional or future-only.

The retained legacy class comprises blocked, previously attempted, and branch-only incidents; a new report remains admitted when the consumer is blocked, previously attempted, or branch-only.
...
Attempts are append-only closed building/reviewed records. Attempt 1 starts at the admission baseline; attempt 2 starts only from attempt 1's exact preserved-scope REJECT reviewed commit and retains its complete paths, review, and dispatch authority. Reviewed attempts bind full candidate commit/tree, sorted Git-observed no-renames paths, fresh review ref/hash/commit/tree, and exact dispatch claim/closure. The planned state graph, 16 explicit variants, six blocked origins, fixed refs, claim observations, and field sets are exactly those in the machine-readable contract in `CONTINUATION-SCOPE-DESIGN.md`. Integration binds deterministic staged ref/worktree/commit/tree; verification binds the passing execution; publication binds exact baseline-to-integration old-OID ref mutation and time. Generic and legacy authority are mutually exclusive. Persisted legacy records remain readable/progressable; generic parity retires new legacy admission only for pristine-pending baseline-substrate incidents, while blocked, previously attempted, or branch-only cases remain a disjoint narrowed class. Any generic claim or record rejects v1/v2 continuation at every construction, allocation, publication, adoption, replay, and resume boundary.
...
The `run.json` root is closed to exactly `schema_version`, `run_id`, `mode`, `status`, `created_at`, `updated_at`, `heartbeat_at`, `base_ref`, `base_commit`, `branch`, `worktree`, `github_account`, `pr_mode`, `pr_url`, `max_parallel_slices`, `max_retries`, `review_tier`, `debug_snapshot`, `provenance`, `merged_slice_repair`, `special_builder_dispatch`, `continuation`, `checkpoint_source`, `checkpoint_progress`, `steering`, `post_pr`, `gates`, `slices`, `cost_attribution`, `steps`, `validator`, `security_review`, and `terminal_result`. Unknown root keys have no legacy fallback. `schema_version` is required and equals `1`. `checkpoint_source` is allowed only on a normal checkpoint child or same-checkpoint B1 descendant; `checkpoint_progress` is allowed only on the blocked routing parent, and the two are mutually exclusive. This slice validates the three top-level timestamps as timestamps and requires top-level `worktree`, when present, to be absolute.
...
Continuation version selection is explicit: unflagged and post-PR children retain continuation schema v1, while eligible pre-PR `--carry-forward` children use continuation schema v2 and the closed `carry_forward` record above. In both cases root `run.json.schema_version` remains 1. The schema-v1 paragraph below remains authoritative for its narrow adoption path; ordinary v2 atomically publishes its attempt-zero inherited spec acceptance and does not call `adopt-continuation` afterward, while checkpoint-bound v2 publishes no spec acceptance at all.
...
`integration_gate` is closed to exactly `required_commands` and `timeout_ms`. The timeout is an integer from 1000 through 1800000 milliseconds; new plans normally use 600000. The command value is an ordered list of 1-32 closed entries containing exactly `program` and `args`; these are structured argv, never shell text. `program` must equal its trimmed form, contain 1-255 UTF-8 bytes, and contain no NUL/control character. `args` contains 0-64 strings per command; every arg is at most 4096 UTF-8 bytes and contains no NUL. The JSON-encoded `required_commands` list is at most 65,536 UTF-8 bytes. The exact entry `{ "program": "npm", "args": ["run", "check"] }` occurs exactly once and is final. Preceding entries are named acceptance commands. JSON, list order, and timeout are authoritative; human `plan.md` text mirrors them only. Existing plans without timeout retain the legacy 300000 millisecond semantic default. New claims and receipts persist the effective timeout, and replay rechecks it against exact accepted plan authority.
