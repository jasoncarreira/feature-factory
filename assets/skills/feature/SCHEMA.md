...
Every slice review carries `remediation_context: {schema_version: 2, fixes}`. `fixes` has exactly one ordered `{required_fix_index, classification, scope_effect, likely_paths, fix_owner}` for every `required_fixes` entry; missing context, duplicate positions, extra fields, unknown values, reordered fixes, and schema version 1 reject before review publication. This discriminator selects only ephemeral implementer context and grants no merge, test, acceptance, lane, or mutation authority. Every published slice review also carries the closed pathless `ownership_ratification: {schema_version:2,kind:"factory-derived-modified-extension"}` object. Paths, rationales, and unknown keys are forbidden: checked Git plus exact evidence disclosure are the sole current path authority. Forecast `likely_paths` never grants authority. Fresh tasks receive exact bound prior review/evidence refs, hashes and bytes, attempt/head/path/test observations, current slice/lane/branch/worktree/head, and authorized brief/research refs as fenced untrusted input. Builders must re-observe Git/files and verify the correction.

B3.4 keeps remediation feasibility separate from successful finish-and-disclose publication. Sibling-owned and contract-change fixes from a `REJECT` still route to the owner, generic integration amendment when eligible, schema-v2 full-plan carry-forward when non-pristine, or plan/brief amendment under the current rules. Pending/running/review siblings route to that sibling owner and never grant ordinary ratification authority. Checked builder context identifies exact `declared_paths`, current reviewed `effective_paths`, and forecast paths, but forecasts never grant mutation or merge authority. For every Git-observed concrete path outside declared ownership, slice evidence contains sorted unique `ownership_disclosure: [{path,rationale}]` whose path set exactly equals the observed unexpected set and whose rationale is nonempty trimmed NFC-normalized text. New reviews use pathless `ownership_ratification: {schema_version:2,kind:"factory-derived-modified-extension"}`; the factory, never the reviewer, derives eligible paths and retains each exact evidence rationale. A v2 `APPROVE` may admit only a newly added zero-owner private regular file or a content-only modification of an unchanged-mode private regular file with zero owners or one current accepted non-touching sibling owner. Delete, rename/copy endpoint, mode/type change, symlink, submodule, generated/privileged, ambiguous, touching, missing, stale, partial, or cross-bound authority denies publication. The shared fail-closed privileged/control-plane workflow policy covers CI, agent/skill/command assets, dependency manifests, migration artifacts, and generated artifacts; each requires explicit declared plan ownership.

Production builder dispatch consumes explicit discriminators through the plugin `tool.execute.before` hook. Every ordinary slice-attempt `backend-builder` or `frontend-builder` Task prompt starts with `FEATURE_FACTORY_SLICE_DISPATCH {"run_id":"<run-id>","slice_id":"<slice-id>","attempt":<N>,"agent":"<builder-agent>"}`. Under the run lock, the hook derives a `PLUGIN_CHECKED_SLICE_CONTEXT_START` block from exact accepted plan/input bytes and review bindings, current run/slice/Git authority, and bound prior review/evidence bytes; remediation HEAD must equal the prior `reviewed_commit`. Every checked context is transported only as base64url-encoded UTF-8 JSON so model-authored `@file`, `@agent`, or command-like strings cannot become OpenCode prompt controls. Before release the hook create-publishes one immutable `dispatch/<sha256-run-slice-attempt>.json` claim binding run, slice, attempt, role, branch, worktree, HEAD, and checked-context hash, then re-observes the complete snapshot. The claim contains no `task_id`; it is a cross-session/process duplicate fence and crash tombstone, so an existing claim fails closed rather than redispatching the attempt. Runtime `task_id` reuse additionally requires a plugin-captured binding to the same live session, role, slice, branch, worktree, and immediately prior attempt plus a checked all-`narrow-correction` prior review. Alternate-role dispatch rejects, and selecting any fresh remediation task invalidates every older runtime binding for that slice. The arbitrary caller body is base64-fenced as untrusted detail beneath a plugin-generated canonical directive; it never shares authority with the checked context. Every fresh integration amendment, panel remediation, or post-PR remediation builder Task instead starts with `FEATURE_FACTORY_SPECIAL_BUILDER_DISPATCH` plus exact run, route, and agent JSON. The plugin rejects every unmarked builder and injects `PLUGIN_CHECKED_SPECIAL_BUILDER_CONTEXT_START` only after re-observing route-specific refs, hashes, bytes, ownership, and clean Git identity. Special routes receive no `task_id` and cannot satisfy ordinary slice dispatch authority. Each route instance has a deterministic create-only `.special.json` claim and capability-authenticated `.special.closed.json` foreground closure; failed, promoted-background, duplicate, concurrent, or crashed Tasks remain unresolved and fence every run mutation, terminal path, PR path, and continuation across processes. These runtime markers are not durable schema fields.
...
OpenCode compaction exact-matches the child session's original injected prompt. Post-compaction model calls receive freshly revalidated checked context through plugin-owned system text; the model-authored compaction summary is non-authoritative. When an active claim has no closure and its worktree has a new clean descendant HEAD, `factory slice-dispatch-adopt <run-id> <slice-id> <attempt> --json` derives that exact claim and candidate, then create-publishes `checked-slice-builder-dispatch-adoption` at the same exclusive `.closed.json` ref without a completion token. The adoption record intentionally carries no operator or callback provenance. The slot permits exactly one completion kind: normal callback closure or candidate adoption. Both bind `dispatch_closure_ref`, `dispatch_closure_hash`, and `completion_head`, but adoption grants no evidence, test, review, ownership, acceptance, or merge authority; all downstream checks remain unchanged. Unknown completion kinds or widened key sets reject.
...
The launcher validates this metadata and maps it into child-process env for opencode: `--traceparent` sets `TRACEPARENT` and `FEATURE_FACTORY_TRACEPARENT`; `--tracestate` sets `TRACESTATE` and `FEATURE_FACTORY_TRACESTATE`; `--parent-span-id` or the span id inside `--traceparent` sets `FEATURE_FACTORY_PARENT_SPAN_ID`. A requested, resumed, or continuation target run id is also passed as process-local `FEATURE_FACTORY_RUN_ID`, but it remains correlation candidate data until it matches the exact nonce-bearing live launch claim or checked durable dispatch/provenance evidence. It is runtime metadata only and is never durable workflow authority. Existing operator-provided `OTEL_EXPORTER_OTLP_*` and `OTEL_RESOURCE_ATTRIBUTES` env is preserved.
...
## Runtime-Only B6 Session Correlation

The supported seam is the public OpenCode **1.18.3** plugin API. `command.execute.before` validates the encoded factory command envelope, but its run id remains an untrusted candidate. Binding requires the exact nonce-bearing live launch claim, a checked durable builder/amendment dispatch, or an exact hash-and-byte-count-bound review provenance record; a session lifecycle event alone never binds launch identity. Invalid and unencoded commands bind nothing. `event({event})` exposes session lifecycle information: `session.created`, `session.updated`, and `session.deleted` carry `properties.info.id` plus optional `parentID`, while `session.status`, `session.idle`, and `session.compacted` carry `properties.sessionID`. Observed children inherit only verified bounded run identity from their parent. `tool.execute.before/after` exposes Task `sessionID` plus `callID`; the ordered pair identifies one hook invocation. Session parentage is best-effort from those fields. Prompt bytes may be hashed to match prior durable provenance but prompts, messages, arguments, results, reviews, evidence, and model output are never parsed for correlation identity or exported.

The hooks expose no native span handle and no native OpenCode span-context extraction/enrichment API. B6.2 emits adjacent factory-owned spans only through `withSpan()`. It cannot mutate or enrich native OpenCode/AI SDK spans and must not claim otherwise. W3C linkage comes from active OpenTelemetry context or extraction of validated launcher `FEATURE_FACTORY_TRACEPARENT`/`FEATURE_FACTORY_TRACESTATE` process metadata into a remote parent context; the event/tool hooks do not expose native context.

`createSessionCorrelationProbe()` is a non-emitting test/feasibility seam used directly by `test/plugin.test.js`. Every explicitly telemetry-enabled plugin instance owns a process-local map bounded to 256 sessions and 512 active calls; disabled instances keep no map. A separate process-local bridge, bounded to 32 repositories and 256 sessions per repository, lets independently instantiated command and Task hooks share only launch-claim- or durable-dispatch-verified run identity for the exact same session or a child linked by an observed parent-session event. A checked durable builder, integration-amendment dispatch, or exact review-provenance match may promote its factory-observed run ID to that exact session. A conflicting reparenting drops correlation, and an invalid or unencoded feature command clears the exact command-session binding through a tombstone retained across session deletion until a later verified command replaces it. Shared-registry capacity eviction advances a process-wide generation that clears retained local correlation before another event or Task can use it. Every ordinary reviewer dispatch treats either a tombstone or a missing/evicted/unverified bridge entry as an instruction to clear local correlation unless exact durable review provenance matches, so bounded capacity always fails closed instead of resurrecting stale identity. Sessionless commands, unrelated sessions, prompt text, and model output never supply run identity. The maps store only terminal-safe session/call/run IDs, observed parent session, closed lifecycle event, target agent, and before/after state. Oldest entries are evicted, retained Task spans have a one-hour safety bound, production session spans are deduplicated to one per lifecycle event kind per session, and session deletion/eviction removes that session's local calls. Restart starts empty; nothing is persisted or exported. The probe ignores malformed IDs and its own failures so telemetry observation never changes workflow behavior.

Canonical B6.2 fields are exactly `feature_factory.run_id`, `feature_factory.slice_id`, `feature_factory.attempt`, `feature_factory.verdict`, `feature_factory.convergence`, `feature_factory.session_id`, optional observed `feature_factory.parent_session_id`, `feature_factory.call_id`, `feature_factory.call_relationship`, `feature_factory.target_agent`, `feature_factory.route`, `feature_factory.lane`, `feature_factory.task_context`, `feature_factory.continuation_kind`, `feature_factory.span_event`, and `feature_factory.span_operation`. `session_id` plus `call_id` is the composite Task-hook key. `call_relationship` is `task-hook|parent-session`; parent session alone does not identify one of several concurrent calls. `task_context` is `fresh|reuse`; `continuation_kind` is `full-plan-carry-forward`. `span_event` is `session-created|session-updated|session-deleted|session-status|session-idle|session-compacted|task-before|task-after`; `span_operation` is `observe-session|execute-task`. Runtime builder `task_id` reuse remains an ephemeral authority check but raw `task_id` is never telemetry identity, stored in the map, persisted, attached to a span/event, or exported.

Identifiers use ASCII `[A-Za-z0-9][A-Za-z0-9._:-]*` and are at most 128 UTF-8 bytes. Values classified as credential-shaped or high entropy are emitted only as fixed SHA-256 pseudonyms, consistently across workflow and opaque session/call identifier fields. Per-site enum fields are closed to at most 32 printable ASCII tokens, each at most 32 bytes. Workflow verdicts retain their canonical uppercase ASCII spellings (`APPROVE`, `REJECT`, `GO`, `GO-WITH-NITS`, `NO-GO`, `PASS`, `BLOCK`, `APPROVE-CHECKPOINT`, and `REDESIGN-REQUIRED`); every other enum token is lowercase ASCII. Unknowns are omitted, never forwarded as arbitrary strings. Attempt is a bounded integer from checked factory state. B6 factory-owned correlation spans/events categorically exclude artifact, review, and evidence refs; content hashes; raw gate answers; prompts and messages; tool arguments/results; review and evidence bodies; raw paths and URLs; secrets; `traceparent`; `tracestate`; arbitrary model output; and any other captured content. General feature-factory or native OpenCode/AI SDK content-capture enablement cannot widen this B6 policy. No dotted or duplicate compatibility aliases are emitted. Telemetry is explicitly enabled, best-effort, and non-authoritative. B6.1 performs no production span emission.
...
The effective CLI and configured plugin identities are closed `{source, version, hash}` observations. Install and doctor expose the effective PATH CLI tuple; snapshots expose configured `plugin_identity` and PATH `cli_identity`. Run creation and mutating resume store them only under `debug_snapshot.created_with.env` and `debug_snapshot.last_resumed_with.env`. They are diagnostic-only, expose no config content, and are never top-level state, provenance, or gate/review/merge/PR/launch authority.

Effective plugin config follows child `HOME`, effective `XDG_CONFIG_HOME/opencode`, project files from cwd through the nearest Git root and project `.opencode`, and non-empty `OPENCODE_CONFIG_DIR` resolved from cwd. One readable local `file://` registration may name the package root, current `src/opencode-plugin.js`, or legacy `src/plugin.js`; multiple, ambiguous, malformed, unreadable, or symlink registrations fail closed, while bare package registration may use the executing package fallback. Every factory launch route (start, start-as-resume, resume, schema-v2 continue, checkpoint-start, and approval handoff; foreground and detached) requires exact SHA-256 byte equality between configured and executing runtime package closures, then configured `src/cli.js` and PATH `feature-factory`. Each bounded closure hashes `package.json`, every shipped `.js` file below `src/`, and every regular workflow file below `assets/` by canonical sorted relative path plus unambiguous path/content lengths and bytes; symlinks, unreadable or nonregular entries, excessive files/bytes, and incomplete roots reject. Missing or mismatched bytes return `RUNTIME_ADMISSION_FAILED` with the `npm` executable and safe exact argv `["install","--global","--","<observed-package-root>"]` targeting the configured package root. Both closure `{source, hash}` tuples, the shared plugin implementation and package CLI identities, and effective PATH OpenCode absolute source/hash are bound and rechecked; config and all bindings are immediately re-read before spawn, and detached supervision repeats the check after IPC serialization. Config removal, source/byte drift, or disappearance rejects. The bound absolute OpenCode executable is spawned with `shell:false`.
...
feature-factory factory amendment <run-id> report --owner-slice ID --consumer-slice ID --defect-path PATH --artifact-id ID --json
feature-factory factory amendment <run-id> <build|review|integrate|verify|merge|block> --json
...
When a spec review rejects a produced canonical brief, record the unaccepted draft explicitly with `factory step <run-id> spec-writer rejected --attempts N --artifact-ref artifacts/technical-brief.md --review-ref reviews/spec-writer.attempt-N.json --json`. For a nonconvergent review that stops retries while preserving the draft, use the same bindings with status `blocked`. Rejected or blocked planning is not continuation authority; file presence alone never proves acceptance.
...
The current canonical-source manifest has 185 variants, 184 production-covered rows plus the sole future row `final-plan-descriptor`. It includes `terminal-result-blocked-nonconvergence`, independently cataloged ordinary and nonconvergent blocked slices, attempt-history and checked-dispatch authority, the schema-v2 continuation envelope and parent/review/target/context/planning bindings, the delivery envelope, invariant-family ledger, reviewed checkpoint plan, typed valid probe, child disposition, routing artifact, terminal route, creation-only publication claim, immutable source, all five progress variants, merged completion, content-addressed final closure, five verification-artifact claim states, passing/failing verification-artifact receipts, and all 48 current generic amendment rows. Every generated mutation reaches its shared validator, checked consistency reader, checked transition, exact claim/receipt consumer, or downstream authority revalidation seam.
...
|---|---|---|
| Plan and slices graph | `plan-slices-json`; `plan-v2-integration-gate`; `plan-delivery-envelope-v1`; `checkpoint-reviewed-plan-v1`; `checkpoint-admission-probe-valid`; `checkpoint-child-disposition-v1`; `checkpoint-routing-artifact-v1`; `checkpoint-child-publication-v1`; `checkpoint-source-v1`; `checkpoint-progress-reserved`; `checkpoint-progress-child-published`; `checkpoint-progress-launched`; `checkpoint-progress-merged`; `checkpoint-progress-closed`; `checkpoint-merged-completion-v1`; `checkpoint-final-closure-v1`; `final-plan-descriptor` | Graph and reviewed checkpoint-plan identity, typed probe and reviewer dispositions, creation-only publication, immutable child source/configuration, monotonic parent progress, exact same-checkpoint completion, and reservation-free content-addressed closure |
...
| Schema-v2 full-plan continuation | `continuation-v2-envelope`; `continuation-v2-parent-binding`; `continuation-v2-selected-review`; `continuation-v2-target-binding`; `continuation-v2-parent-artifact-sidecar`; `continuation-v2-parent-evidence-sidecar`; `continuation-v2-parent-review-sidecar`; `continuation-v2-planning-reuse` | Parent/child identity, exact selected review and context bytes, accepted planning, closed configuration, current plan hash, immutable accepted rows, and disjoint nonempty remaining slice IDs |
...
| Generic integration amendment | exactly 48 current rows: 16 explicit manifest variants; 12 report/verify claim variants; 12 report/verify receipt variants; two review variants; four builder-dispatch variants; amendment-review dispatch claim and closure | Fixed pre-manifest report tombstone; factory-owned baseline/staged failing-to-passing reproduction; exact owner and pristine-pending consumer snapshot; append-only attempts; fresh reviewed commit/tree and Git-observed paths; immutable checked reviewer callback provenance; staged integration; old-OID feature publication |

The current generic integration amendment within class 9 has exactly 48 current rows and is the sole in-place repair authority. Continuation, checkpoint, blocked, previously attempted, branch-only, and other non-pristine consumers reject before effects and require checked terminalization followed by fresh schema-v2 full-plan carry-forward.

Generic amendment review bytes are not orchestrator-authored evidence. The plugin accepts only `FEATURE_FACTORY_INTEGRATION_AMENDMENT_REVIEW` for a fresh foreground `work-reviewer` with no `task_id`, background execution, or delegation. Its immutable nonsemantic claim binds the exact run, amendment, attempt, baseline, candidate commit/tree, fixed review ref, complete all-slice ownership snapshot, no-renames changed paths, probe/failure context, and attempt-2 prior fixes through the checked context hash. Only the exact same-session/same-call/same-agent callback may create-publish the closed seven-disposition review and immutable closure. Preexisting review bytes reject before dispatch; publication never replaces bytes; exact callback replay verifies rather than overwrites. This claim/closure is the narrow amendment-review provenance exception because ordinary semantic provenance and run writers remain fenced until the amendment is merged.
...
Slice sources use five persisted statuses but six independent catalog variants because ordinary and terminal-nonconvergent blocked rows are distinct. They contain no synthetic nested `review_binding`; the top-level `{evidence_hash, review_hash, reviewed_commit}` fields are required current review/merge authority beside `evidence_ref` and `review_ref`. B2 adds required append-only `attempt_reviews`. Entries are `{attempt, evidence_ref, evidence_hash, review_ref, review_hash, reviewed_commit, verdict, convergence, remaining_fix_count, dispatch_claim_ref?, dispatch_claim_hash?, dispatch_closure_ref?, dispatch_closure_hash?}`; the dispatch quartet is all-or-none. Every newly started attempt also carries `dispatch_required: true`; checked dispatch atomically binds the all-or-none claim tuple `{dispatch_claim_ref, dispatch_claim_hash}`, and confirmed successful synchronous foreground return binds the all-or-none closure tuple `{dispatch_closure_ref, dispatch_closure_hash}`. A closure requires its claim, pending rows carry none of these fields, and review/merged rows with `dispatch_required` require both exact tuples. Review publication retains that tuple in history before the next attempt clears the top-level current tuple. Every entry binds exact sidecar bytes and heads; attempts are strictly increasing from the fixed 1-through-3 range, and the current review/merged tuple equals the final entry for its attempt. Old slice review/merged rows and incomplete history reject without mutation. The current validator source is exactly `{verdict, report, report_hash, review_ref, review_hash, reviewed_head_sha}` and the current security source exactly `{verdict, review_ref, review_hash, reviewed_head_sha}`. Both panel rows use their complete current tuples or are both absent; zero or partial present tuples reject. External-operation identity lives in `run.steering.boundary`, `run.steering.action_claim`, and `run.steering.last_action`, each with its real token/kind/generation/time fields (plus boundary state hash or last-action outcome as applicable). Post-PR revalidation jobs retain the production `action_token` field required by running jobs; no token is invented at the `post_pr` root, remediation container, dispatch, or push.

Post-amendment slice start adds immutable `authorized_baseline_commit` to the existing durable slice variants without adding a catalog row. The checked pending-to-running writer derives it from the exact clean current feature HEAD and proves the supplied first-attempt branch and registered clean worktree equal that commit before publication. First dispatch and every history `diff_base_commit` bind it; retry dispatch binds the immediately prior reviewed commit. Merge and post-merge consistency require it to equal the exact Git merge base and require the ownership-reviewed path set to equal the integrated path set.

B3.2 extends every durable slice variant with required immutable `declared_paths`, copied byte-for-byte from the exact accepted plan's `paths`, and required `effective_paths`. Pending, running, blocked, and current `REJECT` rows have `effective_paths` exactly equal to `declared_paths`; current `APPROVE` review/merged rows append exactly that current attempt's `ratified_paths`. Prior attempts are never unioned. Every successor attempt-history entry carries `ownership_schema_version: 2`, sorted unique `ratified_paths`, and sorted unique `modified_extensions`. For `REJECT`, factory-derived ratification is empty: `modified_extensions` and `ratified_paths` are empty arrays. Each unowned record is exactly `{kind:"modified-extension",path,rationale,authority:"unowned"}`. A non-conflicting-sibling record has those common fields with `authority:"non-conflicting-sibling"` plus exact `owner_slice_id`, `owner_attempt`, owner evidence/review/dispatch claim/dispatch closure ref-hash pairs, `owner_reviewed_commit`, and `owner_diff_base_commit`. Its rationale is copied exactly from the modifying evidence disclosure. `ratified_paths` is the compatibility/effective-ownership projection and exactly equals `modified_extensions.map(entry => entry.path)`. A slice may retain immutable v1 entries and append v2 entries; one entry never mixes variants, and no v1 entry is rewritten, migrated, or backfilled. Historical v1 sidecars remain readable and already-published v1 review rows remain mergeable only under their original newly-added unowned policy; new checked publication rejects v1 so reviewer-supplied paths grant no successor authority. Every entry also retains the immutable first checked `diff_base_commit`; when `authorized_baseline_commit` is present it equals that history base and exact merge base.
...
The implemented current generic integration amendment keeps root schema version 1 and uses the optional closed `integration_amendment` introduced in B5.2. Its common keys are `schema_version`, `kind`, `amendment_id`, `status`, `owner_slice_id`, `consumer_slice_id`, `defect_path`, `verification_artifact_id`, `admission`, `failure_execution`, `max_attempts`, `attempts`, `created_at`, and `updated_at`, plus only state-specific `integration`, `verification`, `publication`, or `blocked`. `kind` is `integration-amendment`; `max_attempts` is exactly 2. `admission` binds exact baseline ref/commit/tree/worktree, a closed accepted artifact probe, the complete merged-owner acceptance snapshot, and a byte-identical pristine-pending direct consumer. No consumer branch/overlay and no second plan hash is persisted. After amendment merge, that admission snapshot stays immutable while the admitted consumer advances through checked ordinary slice states. Every write revalidates its current and historical builder claim/closure, evidence, review, and merge authority, and accepts feature movement only along the exact recorded first-parent checked-slice merge chain rooted at the amendment integration commit.

Failure and verification bindings are closed `{claim_ref,claim_hash,receipt_ref,receipt_hash}` objects. A fixed per-run report claim is the pre-manifest singleton tombstone and is never selected by caller IDs. Claims are exclusive-create active records completed as pass/fail or one of three unknown reasons: process outcome, authority change, or receipt publication indeterminate. Receipts independently cover pass, nonzero exit, signal, launch error, timeout, and output limit. Report accepts only a completed nonzero-exit failure after exact post-process authority and cleanliness observation; verification accepts only completed pass at the staged integration commit. Other decided report outcomes settle the tombstone without an amendment; unknown, unconsumed, cross-bound, and orphan outcomes fence according to the canonical observation table and have no supported clear/retry.

Attempts are append-only closed building/reviewed records. Attempt 1 starts at the admission baseline; attempt 2 starts only from attempt 1's exact preserved-scope REJECT reviewed commit and retains its complete paths, review, and dispatch authority. Reviewed attempts bind full candidate commit/tree, sorted Git-observed no-renames paths, fresh review ref/hash/commit/tree, and exact dispatch claim/closure. The current state graph, 16 explicit variants, six blocked origins, fixed refs, claim observations, and field sets are exactly those in the machine-readable contract in `CONTINUATION-SCOPE-DESIGN.md`. Integration binds deterministic staged ref/worktree/commit/tree; verification binds the passing execution; publication binds exact baseline-to-integration old-OID ref mutation and time. Generic amendment is the sole in-place repair authority. Continuation, checkpoint, and non-pristine cases require fresh schema-v2 full-plan carry-forward. Any generic claim or record rejects continuation at every construction, allocation, publication, adoption, replay, and resume boundary.

The implemented generic class-9 catalog applies all twelve required mutation families to every one of the exact 48 manifest, claim, receipt, review, builder-dispatch, and reviewer-dispatch variants. Each family in the machine-readable contract assigns its writer, complete reader set, and named structural/runtime/mutation tests to every explicit member. Runtime callback session/call/task IDs are excluded because they are process-local selectors; the completion capability is retained only by the immutable claim/closure binding. Reviewer inventory is exactly absent, active-claim-only, review-published-without-closure, closed-unconsumed, consumed, or orphan/cross-bound. Unconsumed effects fence every semantic transition, only exact review consumes a closed effect, and consumed provenance is revalidated downstream. The exact source-to-sink and behaviorally executed 28-category parity matrices are canonical in `CONTINUATION-SCOPE-DESIGN.md`.
...
named canonical replacements for duplicate internal attestations, and rejects every
old or partial authority shape without upgrade, backfill, or compatibility reading.

## Implemented continuation schema v2 (explicit pre-PR carry-forward)

This section records current Option A(a) / D behavior selected only by `factory continue ... --carry-forward`. The current plan registers its required integration gate and accepted work-decomposer plan/review dependency in the Durable Authority Integrity Catalog above; the remaining carry-forward publication records stay outside these rows. Schema v2 full-plan carry-forward is the only current continuation shape.
        ...
        "attempt_reviews": [
          {
            ...
            "diff_base_commit": "7777777777777777777777777777777777777777",
            "ownership_schema_version": 2,
            "ratified_paths": ["test/adjacent-fixture.js"],
            "modified_extensions": [
              {
                "kind": "modified-extension",
                "path": "test/adjacent-fixture.js",
                "rationale": "The accepted behavior requires this adjacent fixture.",
                "authority": "unowned"
              }
            ],
            "dispatch_claim_ref": "dispatch/B0MR.json",
            "dispatch_claim_hash": "sha256:6666666666666666666666666666666666666666666666666666666666666666",
            "dispatch_closure_ref": "dispatch/B0MR.closed.json",
            "dispatch_closure_hash": "sha256:8888888888888888888888888888888888888888888888888888888888888888",
            "verdict": "APPROVE",
...
The object is closed to exactly `scope`, `plan_ref`, `plan_hash`, `start_commit`, `accepted_slices`, and `remaining_slice_ids`, with `scope: "full-remaining-plan"`. Each `accepted_slices` entry is closed to exactly `id`, `declared_paths`, `effective_paths`, `attempts`, `attempt_reviews`, `evidence_ref`, `evidence_hash`, `review_ref`, `review_hash`, `reviewed_commit`, and `merge_commit`, plus the existing optional `integration_conflict`; ownership arrays retain exact immutable parent values and mixed immutable v1/v2 history is copied byte-for-byte through normalization. V2 rationales and complete sibling provenance are retained, and child publication copies every referenced evidence, review, ordinary dispatch claim/closure, invariant-family receipt/claim, and integration-conflict sidecar at its exact hash. A modifying accepted row containing `authority:"non-conflicting-sibling"` is admissible only when `accepted_slices` also contains that exact owner as a merged row and the recorded owner attempt, evidence, review, dispatch, reviewed commit, and diff baseline equal the owner's current accepted binding. A missing owner, an owner still only in `review`, or stale/cross-bound owner provenance rejects before child publication. `accepted_slices` contains every parent slice whose status is exactly `merged`, in PLAN order. `remaining_slice_ids` contains every nonmerged slice ID, in PLAN order. The arrays are unique and disjoint, and their set union is exactly the complete `slices[].id` set. For PLAN order `[A, B, C]` with merged `A` and `C`, `accepted_slices: [A, C]` and `remaining_slice_ids: [B]` is valid. They need not form a prefix/suffix split. Remaining child rows seed declared/effective paths from the plan and inherit no attempt authority. The deliberate owner/modifier effective-path overlap remains visible, so unchanged sole-owner consumers continue to treat it as ambiguous; carry-forward creates no general overlap exception.

Eligibility is pre-PR blocked only: parent status is exactly `blocked`, with no PR URL/PR-created tuple and no active post-PR observation, remediation, revalidation, push, or continuation, plus `planning_reuse.eligible === true` from accepted planning. Ordinary carry-forward uses the accepted spec-writer planning-reuse shape. A checkpoint-bound carry-forward instead uses the closed `{eligible, plan_ref, plan_hash, review_ref, review_hash}` shape: the refs are exactly `plan/slices.json` and `reviews/work-decomposer.json`, the hashes bind the published checkpoint child's accepted plan/disposition bytes, `plan_hash` equals `carry_forward.plan_hash`, and both hashes equal the immutable `checkpoint_source.child_plan_hash` / `checkpoint_source.child_disposition_hash` authority. Other continuation shapes reject before effects. Each accepted entry must match an exact parent `merged` slice with positive attempts and complete unchanged current evidence/review/reviewed-commit/merge bindings. Actual integration merge order may differ from PLAN and dependency-execution order. The Git first-parent range from `target.base_commit` exclusive through `start_commit` inclusive contains exactly once the set of `accepted_slices[].merge_commit` values, contains no extra commit, and has length exactly `accepted_slices.length`. Each first-parent commit maps by `merge_commit` to one accepted entry and passes that entry's exact two-parent/merge-base/path/object proof. `start_commit` is parent branch HEAD and the last actual merge, or equals `target.base_commit` when `accepted_slices` is empty. This does not require `accepted_slices` order to equal first-parent chain order: PLAN-ordered `accepted_slices: [A, C]` and actual first-parent chain `[C, A]` is valid. Optional parent validator/security evidence is all-or-none complete current binding with both `reviewed_head_sha` values exactly `start_commit`; it is never inherited, and the child always runs a fresh final panel.
...
Before schema-v2 allocation, the route acquires one target reservation at `refs/opencode/continuation-targets/<sha256-child-run-id>` by zero-OID Git CAS. Its canonical blob binds route schema 2, crash-stable `created_at`, and the hash of the complete continuation. A different schema or authority rejects before child mutation; exact crash replay reuses it. Payload, resume, allocation, and publication recheck the reservation.
...
The initial child keeps root `schema_version: 1` and continuation `schema_version: 2`; persists closed mode/account/PR/post-PR configuration, limits and retries of 3, and an absent root `review_tier` for null or the exact non-null checkpoint tier. Ordinary B1 copies exact accepted planning/spec review into an attempt-zero accepted `spec-writer` row. Checkpoint-bound B1 preserves only the checkpoint child's ordinary accepted `work-decomposer` row and exact plan/disposition files: it does not create a `spec-writer` row, technical brief, spec verdict, or spec review. Both variants publish an authority-free `{ "agent": "test-verifier", "status": "blocked", "attempts": 0 }` placeholder, copy exact accepted sidecars, and publish PLAN-ordered immutable merged plus attempt-zero pending rows. The child has fresh steering, `gates: {}`, null validator/security, and no inherited test, panel, PR, cost, provenance, process, or outcome authority. Fresh execution skips bootstrap/story/research/spec/decomposition, executes dependency-ready pending rows, and advances that test-verifier placeholder to a fresh full-plan attempt one only after all rows merge, followed by fresh validator and security reviewers and whole-story pre-PR.
...
```json
{
  ...
  "worktree": "/tmp/opencode/worktrees/app-123-continuation-1",
  ...
  "slices": [
    {
      ...
      "worktree": "/tmp/opencode/worktrees/app-123-short-slug--be-api",
      "authorized_baseline_commit": "0000000000000000000000000000000000000000",
      "attempts": 1,
      "attempt_reviews": [
        {
          "attempt": 1,
...
The `run.json` root is closed to exactly `schema_version`, `run_id`, `mode`, `status`, `created_at`, `updated_at`, `heartbeat_at`, `base_ref`, `base_commit`, `branch`, `worktree`, `github_account`, `pr_mode`, `pr_url`, `max_parallel_slices`, `max_retries`, `review_tier`, `debug_snapshot`, `provenance`, `integration_amendment`, `special_builder_dispatch`, `continuation`, `checkpoint_source`, `checkpoint_progress`, `steering`, `post_pr`, `gates`, `slices`, `cost_attribution`, `steps`, `validator`, `security_review`, and `terminal_result`. Unknown root keys have no fallback. `schema_version` is required and equals `1`. `checkpoint_source` is allowed only on a normal checkpoint child or same-checkpoint schema-v2 descendant; `checkpoint_progress` is allowed only on the blocked routing parent, and the two are mutually exclusive. This slice validates the three top-level timestamps as timestamps and requires top-level `worktree`, when present, to be absolute.
...
Continuation version selection is explicit: only eligible pre-PR `--carry-forward` children use continuation schema v2 and the closed `carry_forward` record above. V1, unflagged, partial, draft-reuse, and post-PR forms reject before effects. Root `run.json.schema_version` remains 1. Ordinary v2 atomically publishes its attempt-zero inherited spec acceptance, while checkpoint-bound v2 publishes no spec acceptance.

Top-level `run.json.continuation` is present only for children created by explicit `factory continue ... --carry-forward`. It requires schema version 2, kind `blocked-run-continuation`, exact parent/review/target bindings, closed configuration, and closed `carry_forward` scope `full-remaining-plan`. V1, partial, draft-reuse, unflagged, and post-PR shapes reject before effects.

For checkpoint-bound schema-v2 carry-forward, the ordinary spec-writer `planning_reuse` shape is replaced, not supplemented, by the closed `{eligible, plan_ref, plan_hash, review_ref, review_hash}` checkpoint variant. It binds the accepted work-decomposer plan/disposition and immutable `checkpoint_source`, and it grants no spec-writer step, technical brief, spec verdict, inherited acceptance, or spec review.

Each `run.json.steps[]` entry may carry an optional `acceptance` binding, written by the accept transition (`factory step <run-id> <agent> accepted`) when the step references an artifact that exists: `{ artifact_ref, artifact_hash, review_ref?, review_hash? }` capture the exact bytes accepted. Schema-v2 carry-forward rehashes this binding and publishes immutable accepted planning only when every current value matches. Missing or drifted binding fails closed; file presence never grants acceptance.

Continuation child runs use the normal run status enum and normal gate/evidence/review schemas. Schema-v2 children skip repeated planning but still run fresh full-plan acceptance tests, implementation-validator, security-reviewer, and whole-story pre-PR after all rows merge. Continuation PRs use the persisted configured PR mode: `draft` creates and records draft PRs, while `ready` creates and records ready-for-review PRs. If remediation is exhausted or the child remains invalid after bounded attempts, write terminal `status: "blocked"` with `terminal_result.pr_url: null` and leave top-level `pr_url` unset.
...
Every slice review sidecar and `attempt_reviews` entry requires boolean `late_discovery_strike`; marker-less records reject fail-closed and are re-seeded rather than upgraded. A consequential attempt-1-discoverable category first raised on attempt 2 must use `late_discovery_strike: true`, remain `converging`, and classify each fix by its actual remediation shape. The strike is hash-bound in `attempt_reviews`, may occur only once, and consumes normal attempt 3 rather than creating attempt 4. A non-strike review may declare genuine implementation nonconvergence on any attempt. Every attempt-3 REJECT must be nonconvergent. Non-slice review policy is unchanged.

A current REJECT with `convergence: "nonconvergent"` and `late_discovery_strike: false` is terminal for autonomous slice retries on any attempt; every final-attempt REJECT must take this form. Requesting the next running attempt re-hashes all bound history, retains the slice as blocked with reason `slice-review-nonconvergent`, and atomically writes blocked `terminal_result.reason: "slice-review-nonconvergent"`. Its closed `nonconvergence` object is `{schema_version: 1, kind: "slice-review-nonconvergence", slice_id, source_review, continuation}`. `source_review` equals the exact current latest append-only attempt entry, never an older historical review. `continuation` is exactly `{program: "feature-factory", args: ["factory", "continue", <parent-run-id>, "--review", <source-review-ref>, "--run-id", "<new-run-id>", "--carry-forward", "--json"]}`. The placeholder is the one operator-selected child identity; no child is created automatically. Changed source bytes, a source not equal to the latest history entry, an alternate parent-referenced review, or any route/schema/argument drift fails before terminal publication and at every later carry-forward construction, publication, adoption, resume, and launch check.

For slice `review` and `merged`, `evidence_hash`, `review_hash`, and `reviewed_commit` are all required; no other slice status may carry them. Those statuses also require `attempt_reviews` through the current attempt, with the final entry equal to the current tuple. Validator and security must both use their complete current tuples: validator requires `{verdict, report, report_hash, review_ref, review_hash, reviewed_head_sha}` and security requires `{verdict, review_ref, review_hash, reviewed_head_sha}`. Missing or partial tuples reject.
...
Current-shape validation is fail-closed. Old-shape slice review/merged rows, incomplete history, partial panel tuples, and old-shape completed runs reject without mutation. Current merged rows are immutable. Pre-PR fence establishment and PR admission re-hash all bound slice/panel sidecars and require both complete panel heads to equal the current clean integration HEAD.
...
Every terminal result is closed to common keys `status`, `run_id`, `pr_url`, `reason`, `summary`, and `artifacts`. A completed result may additionally contain `pr_number`, `pr_node_id`, `repository`, `operation_id`, `head_ref`, `head_sha`, `base_ref`, `base_sha`, and `draft`; no other key is accepted. Old or partial completed tuples reject. Any PR identity field requires `operation_id`, and its complete tuple is `{pr_url,pr_number,pr_node_id,repository,operation_id,head_ref,head_sha,base_ref,base_sha,draft}`. Top-level `run.pr_url` must match. Each `artifacts` value uses the repository-relative durable artifact-ref grammar and cannot be absolute, traverse with `.` or `..`, use backslashes, or select another durable root.
  ...
  "dispositions": [
    {
      ...
      "evidence_ref": "evidence/verification-artifact-<slice-id-sha256-base64url>-<artifact-id-sha256-base64url>.attempt-2.json",
      "evidence_hash": "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
...
`factory artifact-execute <run-id> <slice-id> <artifact-id> --json` is the only checked disposition-evidence producer. New receipt refs are `evidence/verification-artifact-<slice-id-sha256-base64url>-<artifact-id-sha256-base64url>.attempt-<N>.json`; the sibling claim adds `.claim.json`. Both identities are fixed-width SHA-256 base64url encodings of exact UTF-8 identifiers, so spaces, Unicode, slashes, and `..` remain only inside closed subject fields and never become path segments. Before spawn the command create-publishes the claim under the run lock. The active claim is closed to `{schema_version,kind,state,nonce,run_id,slice_id,attempt,plan_ref,plan_hash,head_sha,verification_artifact_id,probe,receipt_ref,claimed_at}` and binds exact parsed program/argv. Completion create-publishes the receipt and atomically replaces the exact active claim with `completed_at`, `status`, and `receipt_hash`; process or publication uncertainty transitions to `unknown` with `failed_at`, `reason`, and receipt status/hash when exact receipt bytes exist. Active/unknown claims require reconciliation and never replay. Exact completed replay spawns nothing and rewrites nothing. Review publication, history, and merge require the exact completed claim nonce, identity, probe, status, receipt ref/hash, and exact receipt bytes; an unclaimed/pre-created receipt, wrong nonce, concurrent contender, stale bytes, or caller-supplied proof cannot grant authority.

Both B4 extension result slots use schema version 1 and are closed. Current plans require `integration_gate.required_commands`, `integration_gate.timeout_ms`, and a complete `delivery_envelope` with per-artifact `timeout_ms`. They produce active `{schema_version,extension,status:"active",grants_b4_authority,decision,reasons}` with a nonempty unique canonical reasons array. Admission decisions are `admit|checkpoint` and grant ordinary seeding authority exactly for `admit`; review decisions are `approve|reject` and grant slice approval authority exactly for `approve`. Plans missing the current gate or envelope reject before an extension result is authoritative.

The routing order is exact: work-decomposer writes the explicit closed `delivery-checkpoint-plan`; the orchestrator runs the nonmutating typed probe from those bytes; work-reviewer returns same-attempt `APPROVE-CHECKPOINT` with one exact ordered `checkpoint-child-decomposition-review` approval per checkpoint; only then does the parent accept and publish routing authority. A valid probe is closed `{schema_version:1,kind:"delivery-plan-admission-probe",status:"valid",decision,plan_ref,plan_hash,reasons,checkpoint_plan_hash,checkpoints}`. An invalid probe is closed `{schema_version:1,kind:"delivery-plan-admission-probe",status:"invalid",decision:null,plan_ref,plan_hash,reasons:[],checkpoint_plan_hash:null,checkpoints:[],errors}` where `plan_hash` is exact or null and `errors` is a nonempty normalized ordered unique list of closed `{path,message}` objects. Invalid probes grant no review or routing authority. Dependency depth above four waves is a valid `checkpoint` reason, not `REDESIGN-REQUIRED`.

The deterministic checkpoint-routing artifact is closed schema version 1 kind `delivery-checkpoint-routing-manifest`. Its source binds exact plan/checkpoint-plan hashes, same-attempt `APPROVE-CHECKPOINT` review ref/hash/attempt/identity, exact typed probe, and active checkpoint admission result. Its checkpoints copy exact acceptance projections, scopes, child plans, hashes, and reviewer dispositions, then add deterministic normal-run requests with complete test/panel/Gate 3/one-PR boundaries. The parent terminal result is valid only as pre-PR `blocked` reason `oversized-plan-checkpoint-routing-required` with exactly one content-addressed `checkpoint_routing` artifact and active empty `checkpoint_progress`.

The blocked routing parent persists closed `checkpoint_progress` `{schema_version:1,kind:"delivery-checkpoint-progress",manifest_ref,manifest_hash,status,entries,final_closure}`. Active progress has null closure. Entries are contiguous and follow `reserved -> child-published -> launched -> merged`; an earlier entry must be merged before a later reservation exists. Common entry fields bind checkpoint/ordinal/root child, branch/worktree, canonical remote-main base, predecessor completion, complete immutable execution configuration, publication claim ref/OID, and reservation time. Child publication adds child run/plan/scope hashes and time; launch adds time; merge adds completed child/hash, source/configuration hashes, exact B1 lineage, canonical merged PR, fresh remote-main observation, and merge time. Closed progress requires all entries merged and exact `{ref,hash,closed_at}` content-addressed closure binding.

The creation-only publication blob is closed kind `delivery-checkpoint-child-publication` and binds parent/manifest/checkpoint/child, exact child branch and absolute worktree, canonical remote main and base commit, predecessor completion, and reservation time. `checkpoint-start` records `reserved`; `reconcileCheckpointPublication` validates the blob OID, atomically creates one publication claim ref plus child branch without replacement, reconciles the registered clean worktree, and serializes exactly `run.json`, `plan/slices.json`, and `reviews/work-decomposer.json` into a complete no-overwrite child directory; the parent then records `child-published`. The claim prevents conflicting creation only and is not a generic writer permit or lifecycle fence after publication.

The child is an ordinary normal run whose extra immutable `checkpoint_source` is closed kind `delivery-checkpoint-source`. It binds parent and manifest, checkpoint/root child, parent plan/review/identity, exact child disposition and typed probe hashes, scope/child-plan/acceptance hashes, and initial canonical base. The ordinary run starts with the accepted child plan/review, pending projected slices, normal test-verifier placeholder, gates/panels/terminal placeholders, and the stored configuration. Launch is ordinary resume; parent progress changes to `launched` only after launch ownership. Thereafter ordinary heartbeat, steering, steps, slices, test execution, panels, Gate 3, PR, terminal, resume, and disrupted recovery apply without a special payload/bootstrap path.

`factory checkpoint-record-merged` is the canonical completion writer. It locks the parent and complete root-to-leaf child chain, allows only schema-v2 B1 descendants carrying byte-identical `checkpoint_source` and complete identical stored configuration, verifies every continuation claim edge, the leaf's complete normal terminal PR tuple, a fresh merged GitHub observation, and fresh remote-main ancestry, then writes immutable `merged` progress. `checkpoint-start` invokes this recovery for a launched predecessor and `checkpoint-close` does so for a launched final child; explicit `checkpoint-record-merged` is the fallback. B1 is forbidden across checkpoints and cannot carry predecessor merged rows, partial PRs, a cross-run train/join, or shared final panels.

Final closure is a reservation-free content-addressed artifact of closed kind `delivery-checkpoint-final-closure`. It binds parent snapshot, manifest and parent plan/review/probe authority, one complete merged completion per checkpoint, fresh remote main, and close time. `checkpoint-close` create-publishes the artifact, then atomically changes parent progress to `closed` with its artifact ref/hash/time; exact replay preserves the historical closure while rechecking ancestry against fresh remote main.

Cleanup is gated, not unconditionally prohibited. `assertCheckpointCleanupEligible` requires exactly one durable merged parent lineage row with the exact target run ID and run hash. `cleanupRun` locks and reobserves that parent authorization before ordinary cleanup. Missing, stale, duplicate, cross-checkpoint, child-published, or merely launched lineage fails closed before deletion.
    ...
    "delivery_units": [
      {
        ...
        "verification_artifacts": [{ "id": "api-tests", "test_plan_index": 0, "test_plan_entry": "npm test -- api", "timeout_ms": 600000 }]
      }
    ]
  },
  "slices": [
...
The plan root is closed to required `slices`, `integration_gate`, and `delivery_envelope`. Every plan reader, newly produced/seeded plan, and schema-v2 carry-forward construction/publication/adoption/local mutation/replay requires all three current roots, explicit gate timeout, and per-artifact timeouts; omission or partial authority rejects and requires re-seeding. Every planned slice is closed to exactly `id`, `stack`, `paths`, `depends_on`, `acceptance`, and `test_plan`; unknown keys have no fallback. `paths` is a nonempty unique list of canonical exact-file or recursive-directory ownership lanes. A stale or non-existent `depends_on` identity is invalid.

`delivery_envelope` is schema version 1 and closed to `schema_version` plus `delivery_units`. Units occur in exact plan-slice order and there is exactly one per slice. Each closed unit has exactly `id`, `slice_id`, `invariant_families`, `obligations`, and `verification_artifacts`; each array is nonempty. Unit, family, obligation, and artifact IDs are lowercase kebab-case (at most 64 characters) and globally unique within each namespace. Families are closed `{id,description}` records. Obligations are closed `{id,description,invariant_family_id,verification_artifact_id}` records and reference one family plus one artifact in that unit. Artifacts are closed `{id,test_plan_index,test_plan_entry,timeout_ms}` records; indexes are distinct non-negative indexes into the unit slice's `test_plan`, `test_plan_entry` exactly equals the indexed string, and new plans carry an integer timeout from 1000 through 1800000 milliseconds. Active admission returns `checkpoint` when at least one unit has more than one invariant family and at least six obligations, or when dependency depth exceeds four waves; otherwise it returns `admit`. Admission does not alter the fixed three-attempt slice budget.

`integration_gate` is closed to exactly `required_commands` and `timeout_ms`. The timeout is an integer from 1000 through 1800000 milliseconds; plans normally use 600000. The command value is an ordered list of 1-32 closed entries containing exactly `program` and `args`; these are structured argv, never shell text. `program` must equal its trimmed form, contain 1-255 UTF-8 bytes, and contain no NUL/control character. `args` contains 0-64 strings per command; every arg is at most 4096 UTF-8 bytes and contains no NUL. The JSON-encoded `required_commands` list is at most 65,536 UTF-8 bytes. The exact entry `{ "program": "npm", "args": ["run", "check"] }` occurs exactly once and is final. Preceding entries are named acceptance commands. JSON, list order, and timeout are authoritative; human `plan.md` text mirrors them only. Missing timeout rejects. Claims and receipts persist the effective timeout, and replay rechecks it against exact accepted plan authority.

`factory slices-seed <run-id> --from plan/slices.json` accepts only that exact run-relative `--from` literal. It resolves `$RUN/plan/slices.json`, rejects an absent, alternate, absolute, repository-relative, escaping, symlinked, or non-regular source, fatally decodes UTF-8 before JSON parsing, applies creation-mode validation, and requires the submitted slice projection to equal the parsed plan. Each durable pending row receives `declared_paths` as an exact copy of its plan `paths` and `effective_paths` as a second exact copy; callers cannot supply either authority. Exact plan bytes/hash are re-observed under the run lock and again at the protected replacement seam. After seeding, plan acceptance is exactly `factory step <run-id> work-decomposer accepted --attempts N --artifact-ref plan/slices.json --review-ref reviews/work-decomposer.json --json`; the closed step `acceptance` object binds the exact plan and review hashes. Missing or old-shape acceptance rejects and requires re-seeding.
...
- `factory slices-seed` and every plan reader enforce the cap. Deeper or old-shape seeded plans reject and require re-seeding; readers do not rewrite persisted plan state.

## Generic Integration Amendment

Generic integration amendment is the sole in-place repair authority. It applies only to a factory-observed baseline failure in one pristine attempt-zero pending direct consumer of an independently approved merged owner. It binds the strict current plan and delivery artifact, complete owner and consumer snapshots, exact failing claim/receipt, append-only attempts, callback-bound fresh review, staged integration, passing verification, and old-OID feature publication. Continuation, checkpoint, blocked, previously attempted, branch-only, and other non-pristine cases reject without mutation, terminalize through the checked class-specific boundary, and proceed only through a fresh schema-v2 full-plan carry-forward child. There is no fallback repair route.
...
PR creation uses the stronger deterministic fence: run `factory pr-fence <run-id> --json` before `gh pr create`. Every present fence requires the complete current `{operation_id,repository,head_ref,head_sha,base_ref,base_sha,draft}` identity tuple; zero or partial identity rejects and requires re-seeding. The fence blocks new steering and every `run.json` writer. `pr-created` rejects a missing, mismatched, or stale fence. Reconciliation derives its child `GH_CONFIG_DIR` only as `join(homedir(), ".config", "opencode-feature-factory", "gh", run.github_account)`, strips every case-insensitive spelling of `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN`, never falls back to global configuration, and performs a shell-free `GET repos/{repository}/pulls?state=all&head={owner}:{head_ref}&base={base_ref}&per_page=100`, strictly normalizing URL, number, node ID, draft, body marker, state/merged time, repositories, refs, and SHAs across at most 10 valid Link pages. Unique exact open normally records; unique exact merged completes without polling; closed-unmerged is `needs-human`; absent on record, ambiguous, malformed/foreign/repeated/incomplete pagination, or other unknown retains the complete fence. Only complete checked absence permits clear.
