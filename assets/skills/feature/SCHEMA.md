...
The active B4 delivery contract supersedes the preceding B2 inventory count: the current canonical-source manifest has 129 variants, 128 production-covered rows plus the sole future row `final-plan-descriptor`. B2 added `terminal-result-blocked-nonconvergence`, independently cataloged ordinary and nonconvergent blocked slices, and extended slice rows with attempt-history and checked-dispatch authority. B4 adds the production-covered `plan-delivery-envelope-v1`, `review-invariant-family-ledger-v1`, `checkpoint-routing-artifact-v1`, and `terminal-result-blocked-checkpoint-routing` variants. Every generated mutation reaches the shared validators, active typed policy evaluators, checked accepted-plan/review observation, checked receipt consumer, checkpoint artifact transition, or terminal-result validator.
...
|---|---|---|
| Plan and slices graph | `plan-slices-json`; `plan-v2-integration-gate`; `plan-delivery-envelope-v1`; `checkpoint-routing-artifact-v1`; `final-plan-descriptor` | Graph identity, dependencies, path lanes, acceptance mapping, ordered required-command authority, and descriptor bindings; delivery-envelope rows add delivery-unit/family/obligation/artifact identity, exact per-slice test-plan bindings, the typed admission-extension decision, and deterministic reviewed checkpoint requests |
...
| Slices and review/evidence bindings | `slice-pending`; `slice-running`; `slice-review`; `review-invariant-family-ledger-v1`; `slice-merged`; `slice-blocked-ordinary`; `slice-blocked` | Actual durable slice keys; ordinary and nonconvergent blocked variants remain independent, while successor rows bind append-only attempt reviews plus exact checked-dispatch claim/closure, evidence/review refs, byte hashes, and reviewed commit authority; `review-invariant-family-ledger-v1` adds delivery-unit/family/artifact mapping, ledger evidence ref/hash, typed probe/result, enclosing reviewed commit, unresolved findings, and the typed review-extension decision |
...
`factory artifact-execute <run-id> <slice-id> <artifact-id> --json` is the only checked disposition-evidence producer. It derives the exact current artifact entry from accepted plan authority, parses it to program/argv without a shell, observes the current running slice attempt and clean worktree HEAD, executes it, and create-publishes `evidence/<slice-id>.artifact-<artifact-id>.attempt-<N>.json`. The closed receipt contains `{schema_version:1,kind:"checked-verification-artifact-execution-receipt",subject,run_id,slice_id,attempt,claim_nonce,plan_ref,plan_hash,head_sha,verification_artifact_id,probe,started_at,completed_at,duration_ms,status,review_ready,commands,result}`. `probe` is exactly `{type:"verification-artifact",verification_artifact_id,test_plan_index,test_plan_entry,program,args}`; `result` is exactly `{type:"verification-result",outcome,summary}`. Pass/fail has exactly one bounded observed command result and `review_ready` is true exactly for observed exit zero; skipped has no command and cannot be review-ready. Publication, attempt history, and merge re-observe exact receipt bytes and require run, slice, attempt, accepted plan hash, reviewed HEAD, artifact ID, command, and result to match the disposition. Arbitrary evidence and caller-supplied proof booleans have no authority.
...
Fresh checkpoint children persist closed `run.checkpoint` `{schema_version:1,kind:"delivery-checkpoint-child",parent_run_id,parent_run_ref,parent_run_hash,manifest_ref,manifest_hash,checkpoint_id,checkpoint_ordinal,child_run_id,base_ref,base_commit,predecessor_checkpoint_id,predecessor_child_run_id,predecessor_merge_commit}`. Ordinal 1 has all predecessor fields null; later ordinals require all three. `parent_run_ref` is exactly `.opencode/factory/<parent-run-id>/run.json`, manifest refs are content-addressed, commits are full lowercase SHAs, and `child_run_id` equals `run.run_id`. `factory checkpoint-start` atomically verifies exact current `main` while zero-OID creating both the child-ID and parent/checkpoint route reservations. For ordinal N it also requires every prior child to have a completed normal PR and verifies the immediate predecessor merge commit contains that PR head and is an ancestor of current exact `main`. Payload decode and resume revalidate parent/manifest hashes, request identity, base ancestry, and both reservations.
