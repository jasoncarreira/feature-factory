...
Fresh checkpoint children persist closed `run.checkpoint` `{schema_version:1,kind:"delivery-checkpoint-child",parent_run_id,parent_run_ref,parent_run_hash,manifest_ref,manifest_hash,checkpoint_id,checkpoint_ordinal,child_run_id,base_ref,base_commit,predecessor_checkpoint_id,predecessor_child_run_id,predecessor_merge_commit}`. Omitted/default `main` and `refs/heads/main` normalize to canonical remote `refs/heads/main`; other refs reject, and top-level `run.base_ref`/`run.base_commit` are required to equal the binding. `checkpoint-start` fetches exact remote main into a temporary checked ref, atomically creates child/route reservation refs plus `refs/heads/<child_run_id>`, and creates/verifies the registered child worktree at `base_commit` before launch. Both checkpoint refs transition together through `reserved`, `launching`, `launched`, or fail-closed `unknown`. Generic Step 0 must reuse these exact resources. Resume repeats canonical remote observation without local-main authority: a descendant advance rejects reserved/launching state as stale before implementation, exact `launched` state preserves its original base/branch across descendant advances, and divergence rejects with explicit recovery routing without rebasing. Existing-child and completed-predecessor authority remains bound to exact pre-existing/launched reservation claims and full pipeline evidence.
