...
| PR79 merged slice repair | `repair-reported`; `repair-repairing`; `repair-review-approve`; `repair-review-reject`; `repair-merged`; `repair-blocked-from-reported`; `repair-blocked-from-repairing`; `repair-blocked-from-review` | Defect path, owner/consumer, plan/owner snapshot, baseline, original and repair evidence bytes, reviewed commit/review bytes and verdict, verification, merge commit/tree, blocked origin, attempts, and quiescence |
...
For PR79 `merged_slice_repair`, the persisted statuses and consequential origin/verdict variants form eight separate entries: reported, repairing, review with `APPROVE`, review with `REJECT`, merged, blocked from reported, blocked from repairing, and blocked from review. Their combined facts are exactly defect path, owner/consumer identity, plan/owner snapshot, baseline, original evidence, repair evidence, reviewed commit and review bytes, review verdict, blocked reason, verification, merge commit and reviewed-tree equality, attempts, and quiescence. The catalog reports completeness only when every listed entry passes its own mutation matrix and independent metadata checks.
