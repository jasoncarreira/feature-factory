...
|---|---|---|
| Plan and slices graph | `plan-slices-json`; `plan-v2-integration-gate`; `plan-delivery-envelope-v1`; `final-plan-descriptor` | Graph identity, dependencies, path lanes, acceptance mapping, ordered required-command authority, and descriptor bindings; `plan-delivery-envelope-v1` adds delivery-unit/family/obligation/artifact identity, exact per-slice test-plan bindings, and the typed admission-extension decision |
