...
The supported seam is the public OpenCode **1.18.3** plugin API. `event({event})` exposes session lifecycle information: `session.created`, `session.updated`, and `session.deleted` carry `properties.info.id` plus optional `parentID`, while `session.status`, `session.idle`, and `session.compacted` carry `properties.sessionID`. `tool.execute.before/after` exposes Task `sessionID` plus `callID`; the ordered pair identifies one hook invocation. Session parentage is best-effort from those fields. Prompts, messages, arguments, results, reviews, evidence, and model output are never parsed for correlation identity.

The hooks expose no native span handle and no native OpenCode span-context extraction/enrichment API. A later B6.2 implementation may emit adjacent factory-owned spans only through `withSpan()`. It cannot mutate or enrich native OpenCode/AI SDK spans and must not claim otherwise. W3C linkage comes only from active OpenTelemetry context and standard propagation in the process calling `withSpan()`; the event/tool hooks do not expose native context.

`createSessionCorrelationProbe()` is a non-emitting test/feasibility seam used directly by `test/plugin.test.js`. Every explicitly telemetry-enabled plugin instance owns a process-local map bounded to 256 sessions and 512 active calls; disabled instances keep no map. It stores only terminal-safe session/call IDs, observed parent session, closed lifecycle event, target agent, and before/after state. Oldest entries are evicted. Session deletion/eviction removes that session's calls and clears child references. Restart starts empty; nothing is persisted or exported. The probe ignores malformed IDs and its own failures so telemetry observation never changes workflow behavior.

Canonical B6.2 fields are exactly `feature_factory.run_id`, `feature_factory.slice_id`, `feature_factory.attempt`, `feature_factory.verdict`, `feature_factory.convergence`, `feature_factory.session_id`, optional observed `feature_factory.parent_session_id`, `feature_factory.call_id`, `feature_factory.call_relationship`, `feature_factory.target_agent`, `feature_factory.route`, `feature_factory.lane`, `feature_factory.task_context`, `feature_factory.span_event`, and `feature_factory.span_operation`. `session_id` plus `call_id` is the composite Task-hook key. `call_relationship` is `task-hook|parent-session`; parent session alone does not identify one of several concurrent calls. `task_context` is `fresh|reuse`. `span_event` is `session-created|session-updated|session-deleted|session-status|session-idle|session-compacted|task-before|task-after`; `span_operation` is `observe-session|execute-task`. Runtime builder `task_id` reuse remains an ephemeral authority check but raw `task_id` is never telemetry identity, stored in the map, persisted, attached to a span/event, or exported.

Identifiers use ASCII `[A-Za-z0-9][A-Za-z0-9._:-]*` and are at most 128 UTF-8 bytes. Per-site enum fields are closed to at most 32 printable lowercase ASCII tokens, each at most 32 bytes; unknowns are omitted, never forwarded as arbitrary strings. Attempt is a bounded integer from checked factory state. B6 factory-owned correlation spans/events categorically exclude artifact, review, and evidence refs; content hashes; raw gate answers; prompts and messages; tool arguments/results; review and evidence bodies; raw paths and URLs; secrets; `traceparent`; `tracestate`; arbitrary model output; and any other captured content. General feature-factory or native OpenCode/AI SDK content-capture enablement cannot widen this B6 policy. No dotted or duplicate compatibility aliases are emitted. Telemetry is explicitly enabled, best-effort, and non-authoritative. B6.1 performs no production span emission.
