...
- If `driver.continuation` is present, classify the invocation as `blocked-run-continuation` only after validating that it names a blocked parent run and an approved review reference. Persist accepted metadata under `run.json.continuation`, treat parent artifacts and reviews as read-only context, keep the parent run unchanged, force `driver.ready = false` for the continuation, run the normal story/brief/build/test/validator/security/pre-PR gates, and create only a draft continuation PR.
