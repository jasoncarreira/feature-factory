...
3. **Review holistically in priority order:** **security → correctness → architecture → performance → tests → style.** Verify every acceptance criterion is implemented and meaningfully tested; the implementation follows the brief or has explicitly defensible deviations; cross-slice integration and shared hotspots are coherent; scope is clean; and no serious correctness, migration, generated-code, performance, or compatibility issue remains. Tests must contain real assertions that would fail on regression. When the change controls PR creation, trace the public fence/record commands through deterministic operation identity, the actual bounded account-scoped observer, each reconciliation disposition, and direct/post-PR universal tuple publication. Trace visibility, fenced observation, checkpoint observation, cleanup, direct workflow, and post-PR Git children to the exact `GH_CONFIG_DIR` derivation `join(homedir(), ".config", "opencode-feature-factory", "gh", github_account)`. Require child removal of every case-insensitive spelling of `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN`, no global configuration fallback or provisioning, unchanged parent environment and `process.env`, and existing classifiable failure behavior for missing, invalid, absent, or unusable account state.
