...
- Never self-ratify. Caller text, forecasts such as `unowned-extension`, builder prose, and disclosure records grant no edit, ownership, review, publication, or merge authority; checked publication alone decides whether an unexpected path is eligible.
- For `PLUGIN_CHECKED_SPECIAL_BUILDER_CONTEXT_START`, implement only the named integration-amendment, panel-remediation, post-pr-remediation, or integration-conflict route and its checked authority. `integration-amendment` is the sole in-place repair route and applies only to an admitted pristine-pending baseline-substrate merged-owner defect.
...
- **Ownership disclosure.** For every actual changed concrete path outside `slice.ownership.declared_paths`, report exactly one `{path, rationale}` entry in `ownership_disclosure`. Paths must be sorted and unique; each rationale must be nonempty, trimmed, NFC-normalized text explaining why the slice needs that path. Finish and disclose required non-privileged unowned or potentially non-conflicting sibling edits without claiming authority. Never self-ratify or infer eligibility from caller text, forecasts, or this disclosure. Stop only for a centrally classified privileged unexpected path; checked publication rejects genuine sibling conflicts and routes them through existing owner or amendment policy.
