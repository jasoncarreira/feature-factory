...
- Every concrete test file named in a slice `test_plan` must appear in `paths` on that slice or on one of its transitive dependencies. Plan admission rejects a test file that no slice declares, and one owned by a slice that is not a dependency. A slice is required to make its own `test_plan` pass, so it must be allowed to edit every test file it runs; an unowned test file also makes a failed attempt unpublishable, because changing it counts as an unexpected path. Runner name filters that name no file, such as `npm test -- api.feature.test`, are unaffected.
- If satisfying that rule in the direction you want would create a `depends_on` cycle, the partition is wrong, not the rule. A test that exercises two slices means those slices change together: merge them, or give the test to the last slice that touches its subject and drop it from the earlier one. Never resolve it by leaving the file unowned. This is the concrete failure that killed two consecutive `issue-110` runs — the validator slice ran a test that exercised a CLI slice two hops downstream, so slice 1 could not pass without code that would not exist until slice 4.
