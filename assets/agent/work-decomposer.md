...
- Declare a test file in `paths` when the slice is expected to create or modify it, and only then. `paths` is the builder's write boundary, not a list of what the slice executes: running an existing regression test grants no authority to edit it, and several slices may run the same stable test without sharing ownership or gaining an edge between them. But when finishing a slice requires changing a test, that file must be declared, because an undeclared change is an unexpected changed path — which is also why a failed attempt that had to touch it cannot publish its failure artifact.
- Apply the previous rule about test-command dependencies to the changed outputs a test exercises, not to the path spelled in the command. If a test exercises changed outputs from a sibling slice, either depend on that sibling, move the test to the last slice that changes what it exercises and drop it from the earlier one, or repartition. If every one of those options would create a `depends_on` cycle, the partition itself is wrong: the slices change together and must be merged. This is the concrete failure that ended two consecutive `issue-110` runs — the slice owning `src/validate.js` ran a test that exercises `src/factory.js`, owned by a slice two hops downstream, so slice 1 could not pass without code that would not exist until slice 4. The declared graph was a clean DAG the whole time; the cycle was visible only through what the test plan exercised.
