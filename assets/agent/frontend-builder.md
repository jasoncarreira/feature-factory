...
- **Ownership disclosure.** For every actual changed concrete path outside `slice.ownership.declared_paths`, report exactly one `{path, rationale}` entry in `ownership_disclosure`. Paths must be sorted and unique; each rationale must be nonempty, trimmed, NFC-normalized text explaining why the slice needs that path. Never disclose or edit sibling-owned, ambiguous, contract, generated, symlink, delete, or rename paths; stop and report the routed owner or amendment need instead.
