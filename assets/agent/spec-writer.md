...
The canonical-source oracle covers every catalog row, not only the core, post-PR, or repair subsets. It independently binds plan/slices, the future-only `final.plan` descriptor, run envelopes and terminal results, the deterministic successor PR fence, the universal PR-created result, every continuation row and sidecar, every post-PR row, and every PR79 repair row. Every current persisted row must have an actual exported `validateRun`, `validateSlicesPlan`, `checkRunConsistency`, or named checked-consumer baseline. Treat `final.plan` only as a future descriptor contract and test its descriptor/ref/hash/external-byte shape without claiming current `validateRun` support. Artifact, evidence, review, continuation, final-plan, and remediation bytes remain external fixture sources; no persisted source may gain `sidecar_bytes`.
