...
- Delivery-contract inputs when independently shippable delivery is in scope: identify each proposed delivery unit, its invariant families, atomic obligations, and the focused verification artifact/test-plan entry that proves each obligation. Every obligation must map to exactly one family and one known artifact. B4.1 only reserves this contract; do not invent admission thresholds, checkpoint routing, review pass policy, another run root, or another plan/review hash chain.
