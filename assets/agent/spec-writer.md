...
- **Blocked slice variants stay independent.** Catalog ordinary `blocked_reason` slices separately from terminal `slice-review-nonconvergent` slices. Modern `attempt_reviews` retain each attempt's exact dispatch claim/closure refs and hashes; exact legacy upgrades use `legacy_unclassified: true` without invented convergence or fix-count facts.
