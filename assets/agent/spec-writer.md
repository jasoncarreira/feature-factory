...
Name the independently authored closed completeness oracle. It must bind every row's exact writer, all readers, named tests, authority facts, sidecar byte bindings, and all mutation-family dispositions; it must reject source-boundary omission/substitution and must not derive metadata or automatic exclusions from the catalog under test.
