...
- Delivery-contract inputs when independently shippable delivery is in scope: identify each proposed delivery unit, its invariant families, atomic obligations, and the focused executable verification artifact/test-plan entry that proves each obligation. Every obligation maps to exactly one family and one known artifact. The runtime owns active admission, checkpoint routing, checked artifact execution, and review pass policy; do not emit policy decisions, checkpoint requests, another run root, or another plan/review hash chain.
...
For the generic integration amendment, register all 48 current variants separately: 16 manifest variants, 12 report/verify claim variants, 12 report/verify receipt variants, two review variants, four builder-dispatch variants, and the reviewer-dispatch claim and closure. Bind the factory-owned baseline failure, pristine-pending consumer and merged owner snapshots, append-only attempts, exact reviewed commit/tree and changed paths, staged integration, passing verification, and old-OID feature publication. Generic amendment is the sole in-place repair authority; old shapes reject fail-closed and continuation, checkpoint, and non-pristine fixes proceed only through fresh schema-v2 full-plan carry-forward.

The canonical-source oracle covers every catalog row, not only the core, post-PR, or amendment subsets. It independently binds current plan/slices and required-command authority, the accepted work-decomposer plan/review step binding, the future-only `final.plan` descriptor, run envelopes and terminal results, the complete current PR fence and universal PR-created result, every schema-v2 continuation row and sidecar, every post-PR row, and all generic amendment rows. Every current persisted row must have an actual exported `validateRun`, `validateSlicesPlan`, `checkRunConsistency`, or named checked-consumer baseline. Treat `final.plan` only as a future descriptor contract and test its descriptor/ref/hash/external-byte shape without claiming current `validateRun` support. Artifact, plan, evidence, review, continuation, final-plan, and remediation bytes remain external fixture sources; no persisted source may gain `sidecar_bytes`.

For post-PR phases `changes-observed`, `committed`, `revalidating`, `validated`, `push-pending`, and `remote-confirmed`, bind `candidate_head_sha`, `remediation_evidence_ref`, `remediation_evidence_hash`, and the actual remediation-evidence file bytes explicitly. Require independent ref, hash, and byte drift plus stale and cross-bound candidate-head mutations. Route file bindings through `checkRunConsistency` and route candidate identity and once-bound changes through the exported checked post-PR transition; a phase is incomplete if it omits any of those five targets.
...
- Repository integration gate -> <ordered structured argv `{program,args}` commands run by test-verifier after all slices merge; final command exactly `{program:"npm",args:["run","check"]}`>
