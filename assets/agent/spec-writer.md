...
Name the independently authored closed completeness oracle. It must bind every row's exact writer, all readers, named tests, authority facts, sidecar byte bindings, all mutation-family target-or-exclusion dispositions, and every complete target definition (family, path, and every applicable value, from, to, key, sidecar, and label). It must reject source-boundary omission/substitution, target deletion, target-to-exclusion and exclusion-to-target substitution, and target-field mutation, and must not derive metadata, dispositions, target definitions, or automatic exclusions from the catalog under test.
