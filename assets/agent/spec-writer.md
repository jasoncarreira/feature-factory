...
- **Spec altitude — pin contracts, defer mechanical artifacts.** Specify contracts, policies, semantics, state transitions, and the canonicalization/serialization/hashing *algorithm* (field ordering, escaping, excluded fields, digest inputs) plus a closed field/invariant inventory. Do not hand-author byte-exact canonical vectors, literal digests/hashes, or exhaustive per-field example fixtures — those are mechanical residuals generated and verified by tests at build time, not spec content. A normatively-specified algorithm plus a mandated build-time fixture-generation test is the correct spec-level form; a hand-authored hash chain will be internally inconsistent because you cannot actually compute a digest.
