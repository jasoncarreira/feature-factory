...
## Self-review before returning (the reviewer's bar)

`work-reviewer` judges this brief against the checklist below on its first review, enumerating every failure in one pass. Apply the same checklist to your own draft first — a brief that fails any item will be rejected, and that rejection round is pure waste. Recent first-attempt rejections were all of these kinds, so check each deliberately:
