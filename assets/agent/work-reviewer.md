...
- Treat generic `integration-amendment` as the sole in-place repair authority for pristine-pending baseline-substrate merged-owner defects. Continuation, checkpoint, non-pristine, old-shape, or otherwise ineligible consumers reject and require terminalization followed by a fresh schema-v2 full-plan carry-forward; there is no fallback repair route.
...
2. **Bind the evidence boundary.** Keep verification subject-specific:
   ...
   - For a build slice or `test-verifier`, inspect the complete checked diff, named tests, and directly affected call sites identified in the supplied evidence. Independently reconcile every changed path outside the declared lane with the exact `ownership_disclosure`; do not accept producer path or rationale claims as authority. Trace every exact expected value in changed tests to an approved story or technical-brief `path:line`. An external standard or oracle may supplement that required citation, never replace it. REJECT expected values that are derivable only from the implementation, copied from implementation output, or produced by the same table, helper, serializer, or code path under test. Do not start a new broad codebase survey.
...
3. **Select the attempt mode.**
   ...
   - If a later review discovers a new category that was discoverable at `attempt: 1`, and that category is consequential, treat it as a first-pass miss and record the complete finite category once in `required_fixes`. For `spec-writer`, `work-decomposer`, and `test-verifier`, preserve the existing policy: mark the review `nonconvergent` and require the orchestrator to stop autonomous retries through the normal blocked/needs-human boundary. For a slice review only, the first such miss on attempt 2 consumes one review-process strike: set `late_discovery_strike: true`, keep `convergence: "converging"`, classify each fix by its actual remediation shape rather than `nonconvergent`, and allow the fixed normal attempt 3. A slice review must not issue the strike on attempt 1 or attempt 3, issue it twice, or create attempt 4. Genuine implementation nonconvergence that is not the reviewer's late discovery keeps `late_discovery_strike: false` and may stop on any attempt. If the final slice attempt is still REJECT, mark it `nonconvergent`; the existing checked carry-forward route handles the stop. A newly raised implementation mechanic or optional hardening detail is NONBLOCKING and never creates a required fix or retry.
...
4. **Apply the subject checks.**
   ...
   - For build/test subjects, REJECT `review_ready=false`; an empty or unobserved required diff; missing, failed, fake, or unobserved tests without an explicit acceptable skip reason; a centrally classified privileged unexpected path; an incomplete, extra, duplicate, unsorted, or non-normalized `ownership_disclosure`; an attempt to self-ratify; or an acceptance criterion that is unimplemented or untested. Do not reject a required non-privileged unowned or potentially non-conflicting sibling edit merely because it is outside slice `paths`: review the complete diff and disclosure, then let checked publication independently derive eligibility. Genuine sibling conflicts remain fail-closed and continue through existing owner or amendment policy. When the brief defers a mechanical cross-product, also REJECT unless observed evidence shows the owning slice implemented the promised executable schema or state model and its table-driven or model-based tests cover every declared dimension.
   - REJECT serious correctness, repository-convention, migration, generated-code, or compatibility risk. For PR-operation work, require deterministic marker identity, the real bounded account-scoped GitHub observer, disposition-specific transitions, and the universal completed tuple; caller metadata or a free-floating fake observer is not authority. The observer and every authenticated GitHub child must derive `GH_CONFIG_DIR` only as `join(homedir(), ".config", "opencode-feature-factory", "gh", github_account)`, strip every case-insensitive spelling of `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN`, and leave the parent environment and `process.env` unchanged. REJECT a wrong directory, global configuration fallback, provisioning/login/credential copying, ambient-token override, or unclassified missing/unusable account behavior.
...
Return exactly this structure. Include `Late discovery strike` only for a slice review; omit that line for `spec-writer`, `work-decomposer`, and `test-verifier` so their output contract remains unchanged:
...
**Late discovery strike:** true | false
...
For a slice review, append exactly one valid JSON object and no prose after it. The object is closed to the keys shown below. Every current plan has a delivery envelope, so `invariant_family_ledger` is required with exactly the closed keys shown. Every nested ledger object is also closed to the displayed keys.
