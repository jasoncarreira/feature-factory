...
3. **Construct bypasses across every touched ingress.** Enumerate **every** ingress the diff touches, including sibling endpoints, and cite `path:line` for each finding. Try concrete attacks rather than merely checking the happy path. Analyze all of these classes:
   ...
   - **Forgeable identity / authz:** client-manufacturable server-owned identity/source/role/permission/trust markers or authz missing on any server path. Deny the untrusted path **before** any trusted-allowance carve-out. For external PR effects, attempt caller-forged URL/number/repository/draft/SHA fields, marker ambiguity, pagination substitution, persisted-account-directory bypass, unsafe clear, and stale local/remote identity.
     For account-bound GitHub effects, require `GH_CONFIG_DIR` to be exactly the validated GitHub login's `join(homedir(), ".config", "opencode-feature-factory", "gh", github_account)` directory. Attempt a wrong directory, ambient `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, or `GITHUB_ENTERPRISE_TOKEN` override, global configuration fallback, automatic provisioning/login/credential copying, and parent environment or `process.env` mutation. Any child-directory mismatch, retained ambient token, fallback, provisioning, or parent mutation is blocking; missing or unusable prepared state must fail through the existing classified path before an unauthorized effect.
