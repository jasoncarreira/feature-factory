...
- A checkpoint route is authorized only by exact accepted plan bytes and their same-attempt APPROVE work-decomposer review. `factory checkpoint-start` requires one canonical GitHub origin, fetches exact remote `refs/heads/main` into a checked temporary ref without mutating or trusting a remote-tracking ref, binds its commit, cleans the temporary ref, and repeats that observation immediately before launch. Child/route reservation refs transition atomically through `reserved`, `launching`, `launched`, or fail-closed `unknown`; existing-child adoption is allowed only from a matching pre-existing `launching`/`launched` claim, never a newly created reservation. Every completed predecessor must have both reservation refs at the same exact claim, with binding equal to `run.checkpoint` and state exactly `launched`, plus the full normal pipeline and canonical GitHub merged PR whose merge commit is an ancestor of canonical remote main. Local-only, stale tracking, forged, non-launched, wrong-origin/repository, raced, or divergent authority rejects.
