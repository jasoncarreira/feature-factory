...
- A checkpoint route uses exact discriminator `delivery-checkpoint-routing-manifest`. `checkpoint-start` owns child bootstrap: its no-replace reservation binds the worktree, atomically includes the child branch, and publishes exact checkpoint/base/branch/worktree run identity before launch. Reserved-to-launching CAS verifies the fetched remote ref, both reservation refs, and branch-at-base after an adjacent registered clean-worktree check. Launch success additionally requires the exact child manifest and owned process/launch outcome. Ordinary resume and disrupted-recovery entry points launch checkpoint children only from exact dual-ref `launched`; `reserved` routes to `checkpoint-start` exact replay and `launching`/`unknown` return reconciliation-required without spawning. Payload decoding accepts any dual-ref `launched` reservation only while the exact child manifest still binds the same checkpoint and top-level base, branch, and reservation worktree; this applies equally to initial and resume payloads. Checked test acceptance and all downstream sinks reobserve identity and require the complete passing claim/receipt/report/review/head tuple with commands exactly equal to the selected manifest request. One authoritative continuation constructor rejects routing parents and checkpoint children before configuration resolution and every dry-run/allocation/publication/launch side effect across schema v1, carry-forward, allocation replay, and new-PR paths. Public status/list/validate treat an exact blocked content-addressed routing parent with empty `run.slices` as intentional only after revalidating terminal, accepted plan/review, admission, and manifest authority; malformed candidates remain invalid and no ordinary depth/projection exemption is inferred from emptiness alone. `checkpoint-close` is the create-only/idempotent final sink and requires exact launched/full-pipeline authority, canonical merged PR, and fresh remote-main ancestry without treating mutable local refs as remote authority. Existing closure replay preserves the exact stored historical record and accepts a newly observed descendant remote tip only while both stored merge and remote-main commits remain ancestors; divergence rejects.
- Every generic checkpoint-child `run.json` mutation requires exact `launched` parent/manifest/dual-ref claim/binding/base/branch/worktree authority before mutation and immediately before atomic replacement. Reserved permits only `checkpoint-start` exact replay; launching, unknown, missing, and cross-bound authority refuse terminalization, recovery, heartbeat start/tick, steering, gates/steps/slices/panels, cost/environment/provenance, fence/PR reconciliation, and direct writers. Bootstrap/start/final-close are dedicated internal transitions. Cleanup fails closed for every checkpoint child and preserves lineage plus reservation refs even under `--force`. Process cancellation and heartbeat stop remain sidecar-only and do not authorize `run.json` mutation.
