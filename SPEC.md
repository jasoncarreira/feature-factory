...
- Every delivery-envelope slice review requires a complete `invariant_family_ledger` with exactly one disposition per current family. Each disposition maps through a current obligation, repeats the enclosing reviewed commit, and binds a checked create-only receipt from `factory artifact-execute <run-id> <slice-id> <artifact-id> --json`. That receipt binds exact run/slice/attempt, accepted plan hash, reviewed HEAD, artifact ID, parsed program/argv, and observed command result; publication, history, and merge re-observe it. APPROVE requires all current outcomes `pass` and no unresolved findings. Missing, duplicate, extra, stale, arbitrary, failed, skipped, wrong-command, or caller-claimed evidence rejects.
- A checkpoint route is authorized only by exact accepted plan bytes and their same-attempt APPROVE work-decomposer review. Its content-addressed manifest and closed blocked terminal result are re-observed at publication and replay. `factory checkpoint-start` atomically verifies current exact `main` while no-replace reserving child ID and parent/checkpoint route, binds the exact parent/manifest/request/base, and for ordinal N verifies all prior completed normal PR children plus an immediate predecessor merge commit containing the predecessor head on current `main`. Fresh checkpoint children persist that closed binding in `run.checkpoint`; payload decoding and every resume revalidate parent/manifest bytes, request identity, base ancestry, and both reservation refs. Each child remains a normal full feature run with its own integration gate, whole-story panels, Gate 3, and exactly one PR.
