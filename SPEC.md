...
- Detached opencode cancellation is run-scoped and fail-closed only when launch had a known explicit run id. `$RUN/process.json` records one verified `opencode-process` identity and `$RUN/processes/<timestamp>.log` records stdout/stderr for that explicit run; if live identity verification is unavailable, the run-owned detached launch fails before writing `process.json`. Generic detached starts without a known run id may have only package-level logs and are not cancellable through `factory cancel`. `factory cancel` sends exactly one targeted `SIGTERM` only after run id, PID, start marker, command, cwd, log ref, and `state:"running"` evidence validate; missing, invalid, stale, mismatched, or non-running evidence returns `status:"failed-closed"`, `signaled:false`, and sends no signal. Broad process kills, process-group signals, `pkill`, and `killall` are outside the contract.
...
Detached process evidence is part of the interrupt contract only for validated run-owned detached launches. A detached `factory resume <run-id>` or validated continuation launch writes `$RUN/process.json` with `{ schema_version, kind:"opencode-process", run_id, execution_id, pid, started_at, updated_at, state, cwd, identity, log_ref, cancel }` and writes run-scoped logs under `$RUN/processes/<timestamp>.log` only after live process identity is verified; unsupported identity inspection fails the launch before `process.json` is written. A generic `factory start --detached "prompt"`, including one with `--run-id <run-id>`, may only write package-level logs: `--run-id` does not grant process-evidence authority over an existing run and must not be documented as guaranteed to write `$RUN/process.json`. `feature-factory factory cancel <run-id> --json` validates run-scoped evidence against the requested run and live process identity. Success sends only `SIGTERM`, updates `process.json.state` to `cancelled`, and returns `ok:true`, `status:"cancelled"`, `signal:"SIGTERM"`, `process_ref:"process.json"`, `signaled:true`, and `updated:true`. Failure is fail-closed: missing/invalid/stale/mismatched/non-running evidence or signal failure returns `ok:false`, `status:"failed-closed"`, `signaled:false`, `updated:false`, a reason, and no broad process kill, process-group signal, `pkill`, or `killall` fallback. `factory cancel` updates only `$RUN/process.json`; it is not a semantic `run.json` state transition and does not use checked run transitions.
