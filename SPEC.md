...
- B5's planned generic integration amendment is a pre-panel/pre-PR successor for one factory-observed failure in a pristine pending direct consumer of an independently approved merged owner. It executes the consumer's exact accepted verification artifact first on the clean feature baseline and then unchanged on the staged owner-only tree; no consumer branch, worktree, or overlay participates. Callers do not supply commands, results, heads, cwd, refs, or hashes. Only a factory-owned completed nonzero-exit failure may create the singleton amendment. Owner-owned production and supporting test/fixture changes remain eligible when the independent review proves every accepted-contract and scope dimension preserved; unowned, sibling, privileged, contract, security, generated, decomposition, conflict, artifact-corruption, and post-integration changes reject.
- A fixed per-run report claim fences the pre-manifest window and permanently classifies active, unknown, settled, unconsumed, consumed, and orphan report authority. The planned amendment preserves distinct recoverable boundaries for reviewed candidate, staged integration, passing reproduction, and exact feature-ref/worktree publication. Final publication uses old-OID `git update-ref` from the observed baseline and accepts only baseline-before-CAS, clean integration replay, or integration-ref plus exact pre-CAS baseline worktree reconciliation. Unknown execution/Task/ref effects stay fail-closed. Persisted legacy PR #79 repairs may finish unchanged; after all 28 adversarial guarantees pass generically, legacy admission retires only for the generic pristine-pending baseline substrate, while blocked, previously attempted, and branch-only incidents remain a disjoint narrowed legacy/recovery class. Generic claims or records cannot coexist or fall back to legacy, and any generic amendment makes v1/v2 continuation unsupported. B5.1 is design-only; these bullets grant no runtime authority until B5.2-B5.4 land.
