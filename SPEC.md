...
- A checkpoint route is authorized only by exact accepted plan bytes and their same-attempt APPROVE work-decomposer review. `factory checkpoint-start` atomically verifies current exact `main` while publishing and compare-and-swap transitioning the child/route reservation through `reserved`, `launching`, `launched`, or fail-closed `unknown`. Exact pre-launch retry and existing-child adoption are idempotent; conflicts, launch uncertainty, and concurrent stale transitions reject without duplicate launch. Every predecessor must prove the full normal pipeline—every slice merged, fresh accepted passing checked test-verifier authority, exact-head passing panels, Gate 3 approval, and a completed exact PR tuple—and canonical GitHub observation must report that exact PR merged at a merge commit containing its head on current `main`. Local-only or divergent merge claims reject. Fresh children persist the closed binding in `run.checkpoint`; payload decode and resume revalidate parent/manifest/request/base/predecessor/reservation authority.
