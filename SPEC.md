...
- Effective plugin config uses child `HOME`, effective `XDG_CONFIG_HOME/opencode`, project config from cwd through the nearest Git root and project `.opencode`, plus non-empty `OPENCODE_CONFIG_DIR` resolved from cwd. A local `file://` registration may target the package root, current `src/opencode-plugin.js`, or legacy `src/plugin.js`; multiple, ambiguous, malformed, or unreadable local registrations fail closed. Bare package registration may use the executing package fallback. Every factory start/start-as-resume/resume/continue/checkpoint/approval-handoff launch, foreground or detached, requires exact SHA-256 byte equality from the configured local package's shared plugin implementation and `src/cli.js` to the executing package, then from configured `src/cli.js` to PATH `feature-factory`. Failure reports the `npm` executable with safe exact argv `["install","--global","--","<observed-package-root>"]` targeting that configured package root. The package CLI and OpenCode absolute source/hash are bound and rechecked together with executing/configured plugin identities immediately before spawn; the detached supervisor repeats the recheck. Config removal or drift rejects. Spawn uses the bound absolute OpenCode executable with `shell:false`.
