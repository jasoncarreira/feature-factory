...
- Current account-bound GitHub execution preserves the exact validated `github_account` spelling and derives `GH_CONFIG_DIR` only as `join(homedir(), ".config", "opencode-feature-factory", "gh", github_account)`. Every spawned GitHub CLI or authenticated Git child receives a copied environment with `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN` removed and fixed noninteractive host/pager values added. The parent environment and `process.env` remain unchanged. There is no XDG, APPDATA, platform-root, parent-directory, case-normalization, provisioning, or global configuration fallback; missing, invalid, absent, or unusable prepared account state retains each consumer's existing classifiable failure/reconciliation behavior.
