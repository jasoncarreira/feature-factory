...
- A checkpoint route is authorized only by exact accepted plan bytes and their same-attempt APPROVE work-decomposer review. Parent/default `main` and `refs/heads/main` normalize to canonical remote `refs/heads/main`; no other ref aliases are admitted. `factory checkpoint-start` fetches that ref into a checked temporary ref, atomically creates child/route reservations plus the child branch, creates/verifies the child worktree at the fetched commit, and repeats remote observation before launch. Child Step 0 must reuse that branch/worktree and persist top-level base fields equal to the checkpoint binding. Resume never consults stale local main: descendant remote advancement rejects reserved/launching preimplementation children but preserves a launched run's original branch/base identity, while divergence rejects with explicit recovery routing and no rebase. Existing-child and completed-predecessor authority retains the exact pre-existing/launched reservation and full-pipeline requirements.
