...
OpenCode compaction continuation is authorized only by plugin-owned system re-injection after exact child-session prompt matching and fresh revalidation of the bound claim, accepted plan/input hashes, slice identity, and descendant worktree identity. A model-authored compaction summary is progress data and grants no authority. A callback that returned without closure stays unresolved unless a trusted local operator explicitly invokes `factory slice-dispatch-reconcile <run-id> <slice-id> <attempt> --authorize-callback-returned-without-closure --json` after independently inspecting, committing, and verifying the work. The strict command accepts no caller-provided ref, hash, completion capability, callback output, session/call identity, or completion commit. It requires the exact active bound claim and a new clean descendant HEAD, then create-publishes a distinct `checked-slice-builder-dispatch-reconciliation` in the claim's exclusive `.closed.json` completion slot. Callback closure and operator reconciliation are mutually exclusive. Reconciliation does not skip focused tests, evidence, independent review, ownership ratification, reviewed-HEAD equality, or merge proof.
