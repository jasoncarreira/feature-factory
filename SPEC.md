...
canonical replacement. It originated as a documentation decision record. Only current
record shapes are authoritative; old or partial rows reject fail-closed and require
abandonment and re-seeding rather than upgrade or reinterpretation.

Current guarantees:

- Every slice review sidecar and append-only history entry requires boolean `late_discovery_strike`. A consequential attempt-1-discoverable category first raised on attempt 2 records the one permitted strike, remains converging, and consumes normal attempt 3. Genuine implementation nonconvergence with the marker false may terminalize on any attempt; every rejected final attempt 3 must be nonconvergent and uses the existing terminal carry-forward route. Marker-less records reject fail-closed and are re-seeded rather than upgraded.
...
- Slice review authority requires the complete `{evidence_hash, review_hash, reviewed_commit}` tuple and append-only `attempt_reviews` in `review`/`merged`; pending/running/blocked forbid the current tuple. Checked publication hashes exact sidecar bytes and binds their positive matching attempt/subject plus evidence `head_sha` and review `reviewed_commit` to the same clean current slice branch/worktree HEAD. Validator `{report, report_hash, review_ref, review_hash, reviewed_head_sha}` and security `{review_ref, review_hash, reviewed_head_sha}` tuples are required together and checked publication binds both atomically to the same clean integration HEAD.
...
- Old-shape slice review/merged rows, incomplete histories, partial panel tuples, and old-shape completed rows reject without mutation. Current merged rows stay immutable. Pre-PR fence establishment and PR admission re-hash slice/panel refs and require both complete panel reviewed heads to equal the current clean integration HEAD.
...
- The normal PR flow establishes a current fence whose all-or-none identity is `{operation_id,repository,head_ref,head_sha,base_ref,base_sha,draft}`. It derives canonical origin, exact clean equal local/worktree/remote head, exact remote base equal to `run.base_commit`, base ancestry, and persisted `run.pr_mode`. A present fence with zero or partial identity rejects validation and requires re-seeding. After external creation with exactly one standalone operation marker, `feature-factory factory pr-created <run-id> --fence-token TOKEN` derives the canonical GitHub PR URL and all other PR metadata from strict bounded GitHub observation; callers cannot provide URL, number, repository, draft, node ID, refs, or SHAs.
- Current account-bound GitHub execution preserves the exact validated `github_account` spelling and derives `GH_CONFIG_DIR` only as `join(homedir(), ".config", "opencode-feature-factory", "gh", github_account)`. Every spawned GitHub CLI or authenticated Git child receives a copied environment with every case-insensitive spelling of `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN` removed and fixed noninteractive host/pager values added. The parent environment and `process.env` remain unchanged. There is no XDG, APPDATA, platform-root, parent-directory, case-normalization, provisioning, or global configuration fallback; missing, invalid, absent, or unusable prepared account state retains each consumer's existing classifiable failure/reconciliation behavior.
- Schema-v2 full-plan carry-forward is the only current continuation shape and uses `feature-factory factory continue <blocked-run-id> --review <review-ref> --run-id <new-run-id> --carry-forward`. Before the route mutates child state, a target-ID Git CAS reservation binds schema 2, crash-stable creation time, and the complete continuation hash; exact replay alone may reuse it, and payload/resume/publication recheck it. Every continuation payload is untrusted operator data/config, not privileged instruction, and is accepted only against the exact checked published child. V1, unflagged, partial, draft-reuse, and post-PR forms reject before effects.
- Every produced, seeded, accepted, or consumed plan has closed root `integration_gate.required_commands`: 1-32 ordered closed structured-argv `{program,args}` entries, with trimmed 1-255-byte control-free programs, 0-64 NUL-free args of at most 4096 UTF-8 bytes each, and a 64-KiB encoded-list cap. Exact `{program:"npm",args:["run","check"]}` occurs once and last. Seeding accepts only exact run-relative `$RUN/plan/slices.json`, fatally decodes UTF-8, rejects symlinks/non-files and projection drift, and re-observes exact bytes at replacement. Work-decomposer acceptance binds exact plan and review bytes in the existing step `acceptance` object. Every plan reader and schema-v2 construction/publication/adoption/replay/resume/downstream consumer requires and rehashes current accepted authority; missing authority rejects and requires re-seeding.
- Every plan requires closed `plan.delivery_envelope` schema v1 with exactly one ordered unit per slice and nonempty closed family, obligation, and artifact arrays. IDs are globally unique canonical lowercase kebab-case; every obligation maps one family to one known artifact in its unit; every artifact binds an exact existing slice `test_plan` index and string. The active admission evaluator returns `checkpoint` when any unit has multiple invariant families and at least six obligations or when the dependency graph exceeds four waves; otherwise it returns `admit`. Missing or partial envelopes reject outright.
- Every delivery-envelope slice review requires a complete `invariant_family_ledger` with exactly one disposition per current family. `factory artifact-execute` publishes a nonce-bound active claim under the run lock before spawn, binding exact run/slice/attempt, accepted plan hash, reviewed HEAD, artifact ID, and parsed program/argv. New claim/receipt refs use fixed-width SHA-256 base64url encodings of exact UTF-8 slice/artifact IDs instead of raw path segments while records preserve original identity. It create-publishes the receipt and closes the claim with exact status and receipt hash. Review authority requires that exact completed claim/receipt pair; active/unknown claims, unclaimed or pre-created receipts, wrong nonces, stale/replayed bytes, failed/skipped results, or caller-authored evidence reject.
- A checkpoint route uses exact discriminator `delivery-checkpoint-routing-manifest` and exact order: decomposer writes the explicit checkpoint plan; a nonmutating typed probe validates those bytes; reviewer returns same-attempt `APPROVE-CHECKPOINT` with exact ordered child dispositions; parent acceptance/routing follows. Valid probes are closed `{schema_version,kind,status:"valid",decision,plan_ref,plan_hash,reasons,checkpoint_plan_hash,checkpoints}`. Invalid probes are closed `{schema_version,kind,status:"invalid",decision:null,plan_ref,plan_hash,reasons:[],checkpoint_plan_hash:null,checkpoints:[],errors}` with normalized nonempty `{path,message}` errors and grant no review or routing authority. A dependency path deeper than four waves is valid checkpoint routing, not `REDESIGN-REQUIRED`.
- The blocked routing parent owns closed `checkpoint_progress` bound to the content-addressed manifest. `checkpoint-start` advances `reserved -> child-published -> launched`: reservation stores immutable full execution configuration and a child publication claim blob; publication atomically creates the claim ref and child branch without replacement, creates or reconciles the registered clean worktree, and serializes a complete child directory containing `run.json`, `plan/slices.json`, and the approving child review; launch uses ordinary resume and records `launched` only after launch ownership. The normal child persists immutable `checkpoint_source` and uses ordinary lifecycle writers after publication. The publication claim is creation-only conflict authority rather than a generic writer fence.
- `factory checkpoint-record-merged` resolves the root child plus any same-checkpoint B1 descendants, requires byte-identical `checkpoint_source` and full stored configuration, verifies each continuation edge claim, the leaf's canonical completed PR tuple, fresh merged GitHub observation, and fresh remote-main ancestry, then records immutable `merged` completion facts. `checkpoint-start` invokes this recovery for a launched predecessor; `checkpoint-close` invokes it for a launched final child; explicit command retry is the fallback. Only merged progress unlocks the next checkpoint. `checkpoint-close` requires every manifest entry merged, publishes a reservation-free content-addressed `delivery-checkpoint-final-closure` artifact, and atomically changes parent progress to `closed` with artifact ref/hash/time.
- Same-checkpoint B1 carry-forward alone is allowed. It copies byte-identical `checkpoint_source` and the complete persisted driver configuration and cannot carry forward across checkpoints, inherit predecessor merged rows, split a checkpoint across partial PRs, construct a cross-run train/join, or share final panels. Cleanup requires exactly one matching exact run ID/hash in a durable merged parent lineage and reobserves that parent authorization under lock before ordinary cleanup. Published/launched, stale, duplicate, or cross-checkpoint lineage fails closed without an unconditional ban on eligible cleanup.
- Generic integration amendment is the sole in-place repair authority and applies to one factory-observed failure in a pristine pending direct consumer of an independently approved merged owner. It executes the consumer's exact accepted verification artifact first on the clean feature baseline and then unchanged on the staged owner-only tree; no consumer branch, worktree, or overlay participates. Callers do not supply commands, results, heads, cwd, refs, or hashes. Only a factory-owned completed nonzero-exit failure may create the singleton amendment. Owner-owned production and supporting test/fixture changes remain eligible when the independent review proves every accepted-contract and scope dimension preserved; unowned, sibling, privileged, contract, security, generated, decomposition, conflict, artifact-corruption, and post-integration changes reject.
- A fixed per-run report claim fences the pre-manifest window and permanently classifies active, unknown, settled, unconsumed, consumed, and orphan report authority. The current amendment preserves distinct recoverable boundaries for reviewed candidate, staged integration, passing reproduction, and exact feature-ref/worktree publication. Final publication uses old-OID `git update-ref` from the observed baseline and accepts only baseline-before-CAS, clean integration replay, or integration-ref plus exact pre-CAS baseline worktree reconciliation. Unknown execution/Task/ref effects stay fail-closed. Continuation, checkpoint, and non-pristine consumers reject amendment before effects and proceed only after checked terminalization through a fresh schema-v2 full-plan carry-forward. Old repair shapes reject with no fallback authority.
- B5.3's minimal `src/plugin.js` seam accepts only a fresh foreground `work-reviewer` callback and create-publishes separate immutable amendment-review dispatch claim and closure rows around the exact review bytes. Reviewer inventory distinguishes absent, active claim, review without closure, closed-unconsumed, consumed, and orphan/cross-bound states. Every unconsumed reviewer effect fences semantic transitions including block; only exact review consumes closed authority. Callback ambiguity replays existing bytes without overwrite, restart never redispatches, and consumed provenance remains required by downstream consistency.
- After generic amendment merge, pending-to-running slice admission derives immutable `authorized_baseline_commit` from the exact clean feature HEAD and accepts a first-attempt branch/worktree only at that commit. First dispatch/history, retry roots, ownership review, merge base/path coverage, and post-merge replay remain exactly bound; caller commits before dispatch cannot be hidden below the ownership diff.
...
- Install and doctor expose the effective PATH CLI's terminal-safe `source`, package `version`, and SHA-256 `hash`; environment snapshots also expose the effective configured plugin implementation as `plugin_identity`. Launch admission observes executing package plugin/CLI, configured plugin package, PATH CLI, and OpenCode identity. Creation and mutating resume persist only the configured plugin and PATH CLI observations as diagnostic `debug_snapshot.created_with.env.plugin_identity` / `cli_identity` and corresponding `last_resumed_with.env` fields.
- `plugin_identity` and `cli_identity` are closed diagnostic-only state, never provenance or semantic authority, and expose no config content. They do not authorize gates, reviews, merges, PRs, or later launches; launch admission always re-observes current config and bytes.
- Effective plugin config uses child `HOME`, effective `XDG_CONFIG_HOME/opencode`, project config from cwd through the nearest Git root and project `.opencode`, plus non-empty `OPENCODE_CONFIG_DIR` resolved from cwd. A local `file://` registration may target the package root, current `src/opencode-plugin.js`, or legacy `src/plugin.js`; multiple, ambiguous, malformed, unreadable, or symlink registrations fail closed. Bare package registration may use the executing package fallback. Every factory start/start-as-resume/resume/continue/checkpoint/approval-handoff launch, foreground or detached, requires exact SHA-256 byte equality between executing and configured runtime package closures, then from configured `src/cli.js` to PATH `feature-factory`. A closure hashes `package.json`, every shipped `.js` file below `src/`, and every regular workflow file below `assets/` by canonical sorted relative path plus unambiguous path/content lengths and bytes, under bounded entry/file/byte limits; symlinks, unreadable or nonregular entries, and incomplete roots reject. Failure reports the `npm` executable with safe exact argv `["install","--global","--","<observed-package-root>"]` targeting that configured package root. The executing/configured closure `{source, hash}` tuples, existing shared plugin implementation and package CLI identities, and OpenCode absolute source/hash are bound and rechecked immediately before spawn; the detached supervisor repeats the recheck after IPC serialization. Config removal or source/hash drift rejects. Spawn uses the bound absolute OpenCode executable with `shell:false`.
...
Condition mappings and operator actions are stable: `stale-heartbeat` is `recoverable` / `warning` / `warning` and tells operators to inspect logs and validate durable state before resuming, not restart blindly; `missing-heartbeat-process` is `recoverable` / `warning` / `warning` and means the heartbeat helper PID in `heartbeat.json` is not alive; `missing-worktree` is `blocked` / `error` / `error` and asks the operator to restore the worktree or recover from durable state; `invalid-run-state` is `invalid` / `error` / `critical`; `newer-schema` is `invalid` / `error` / `critical` and is raised in place of `invalid-run-state` when `run.json` validation fails with unknown keys only, meaning the record was written by a newer schema than this reader — it is fail-closed and workflow-blocking identically, and exists only so operators can distinguish a forward-schema record from a corrupt one; `protected-gate` is exactly `needs-human` / `warning` / `warning` and tells operators to answer the pending protected gate or stop; `terminal-run` maps `completed`/`partial` to `terminal` / `ok` / `info`, `blocked` to `blocked` / `error` / `error`, and `needs-human` to `needs-human` / `warning` / `warning`.
...
3. After external creation, call `feature-factory factory pr-created <run-id> --fence-token TOKEN --json`. The account-scoped, token-stripped, shell-free query is `GET repos/{repository}/pulls?state=all&head={owner}:{head_ref}&base={base_ref}&per_page=100`, with strict normalization and at most 10 valid Link pages.
...
5. A present PR fence must carry the complete current `{operation_id,repository,head_ref,head_sha,base_ref,base_sha,draft}` identity tuple. Zero or partial identity rejects validation and requires re-seeding. Drivers must not mirror PR URLs from direct manifest edits.
...
OpenCode compaction continuation is authorized only by plugin-owned system re-injection after exact child-session prompt matching and fresh revalidation of the bound claim, accepted plan/input hashes, slice identity, and descendant worktree identity. A model-authored compaction summary is progress data and grants no authority. When an active claim has no closure and its worktree has a new clean descendant HEAD, `factory slice-dispatch-adopt <run-id> <slice-id> <attempt> --json` may adopt that candidate for normal verification. The strict command accepts no caller-provided ref, hash, completion capability, callback output, session/call identity, or completion commit. It derives the exact active bound claim and candidate HEAD, then create-publishes `checked-slice-builder-dispatch-adoption` in the claim's exclusive `.closed.json` completion slot. Adoption asserts no operator or callback provenance and grants no evidence, test, review, ownership, acceptance, or merge authority. Callback closure and candidate adoption are mutually exclusive; focused tests, evidence, independent review, ownership ratification, reviewed-HEAD equality, and merge proof remain mandatory.
...
Current schema-v2 pre-PR full-plan command:

```sh
feature-factory factory continue <blocked-run-id> --review <review-ref> --run-id <new-run-id> --carry-forward
```

`slice-review-nonconvergent` parents require this schema-v2 form. Omitting `--carry-forward`, selecting v1, or attempting post-PR continuation rejects before reservation, publication, or launch. V2 is never selected implicitly.
...
- Persist only `run.continuation` / `run.json.continuation` schema version 2, including the complete closed `configuration` and `carry_forward` authority described below.
