...
| Optional closed `delivery_envelope` schema v1 delivery-unit/family/obligation/artifact graph and exact test-plan bindings | `RETAIN` | Retain only inside the already hash-bound exact `plan/slices.json` bytes. Seed and accepted-decomposition observation validate it and re-observe it at replacement; do not create a second run root or hash chain. Omission remains compatible during inactive B4.1, while a supplied envelope must validate. |
...
| Optional closed review `invariant_family_ledger` schema v1, including family/artifact disposition references, evidence ref/hash, typed probe/result, reviewed commit, and unresolved findings | `RETAIN` | Retain inside the existing exact review bytes/hash. Slice-review publication validates references against the accepted envelope, re-observes each ledger evidence hash, and repeats the observation immediately before replacement. The inactive review extension grants no B4 authority and B4.1 applies no approval policy. |
