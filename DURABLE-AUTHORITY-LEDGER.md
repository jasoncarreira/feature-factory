...
| Checkpoint child binding and Git child/route reservation state | `RETAIN` | `factory checkpoint-start` binds exact parent/manifest/request/base/ordinal/predecessor authority in `run.checkpoint`, then atomically creates and compare-and-swap transitions both reservation refs through `reserved`, `launching`, `launched`, or fail-closed `unknown`. Exact pre-launch retry and existing-child adoption are idempotent; conflicts and uncertain launches do not repeat. Predecessor admission re-observes the full child pipeline and canonical GitHub merged PR/merge commit, not local ancestry or terminal shape alone. |
