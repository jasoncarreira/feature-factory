...
| Checkpoint child binding and Git child/route reservation state | `RETAIN` | `factory checkpoint-start` binds exact parent/manifest/request/ordinal/predecessor authority plus canonical fetched GitHub `refs/heads/main`, then atomically creates and compare-and-swap transitions both reservation refs through `reserved`, `launching`, `launched`, or fail-closed `unknown`. Existing-child adoption requires a matching pre-existing launching/launched claim. Predecessor admission reads both refs, requires one exact launched claim whose binding equals `run.checkpoint`, re-observes the full child pipeline and canonical GitHub merged PR, and proves its merge against freshly fetched remote main rather than local or remote-tracking state. |
...
| Checked verification-artifact execution claim and receipt | `RETAIN` | Before spawn, `factory artifact-execute` create-publishes a nonce-bound active claim binding run/slice/attempt/plan/HEAD/artifact/program/argv. Completion create-publishes the receipt and closes that exact claim with status plus receipt hash. Active/unknown, unclaimed, wrong-nonce, stale, concurrent, or replay-divergent evidence is fail-closed and cannot authorize review. |
