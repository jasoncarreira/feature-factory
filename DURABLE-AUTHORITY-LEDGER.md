...
| Checkpoint child binding and Git child/route reservation state | `RETAIN` | `factory checkpoint-start` normalizes only omitted/short/full main to canonical remote `refs/heads/main`, binds its freshly fetched commit, atomically creates both reservation refs and the child branch, and verifies the child worktree at that commit before launch. Child top-level base fields equal `run.checkpoint`. Resume re-fetches remote main: pre-launch stale authority rejects, launched descendant advancement preserves identity, and divergence never rebases. Existing-child and predecessor admission still require exact matching pre-existing/launched reservation authority. |
