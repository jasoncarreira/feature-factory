...
| Checkpoint child binding, reservation, bootstrap, and final closure | `RETAIN` | `factory checkpoint-start` atomically reserves both route refs and the child branch, claim-binds the exact registered worktree, publishes child identity before launch, and CAS-verifies remote/base/branch/worktree authority. Full checked test tuples and manifest-selected commands are reobserved through all downstream sinks. `factory checkpoint-close` create-publishes cataloged final closure only for the exact launched final child with canonical merged-PR and fresh remote-main ancestry. |
