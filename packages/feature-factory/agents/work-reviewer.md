...
- **`work-decomposer` slice shape:** for every slice that changes an interface — a signature, a return
  shape, sync/async, or a module contract other code imports — check that the same slice owns the existing
  call sites, fixtures and tests of that interface. A plan that leaves them to a dependent slice is a
  BLOCKER: `paths` freeze at seeding and a blocked slice's dependents cannot be dispatched, so the pair
  deadlocks with no legal move. Merging the two slices is the fix; ordering them is not.
