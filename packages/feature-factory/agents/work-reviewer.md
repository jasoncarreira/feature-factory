...
- **`work-decomposer` slice shape:** for every slice whose change **invalidates** an existing interface —
  a signature, return shape, sync/async nature, or module contract other code imports, such that existing
  call sites, fixtures or tests must migrate — check that the same slice owns those call sites, fixtures
  and tests. A plan that leaves them to a dependent slice is a BLOCKER: `paths` freeze at seeding and a
  blocked slice's dependents cannot be dispatched, so the pair deadlocks with no legal move. Merging the
  two slices is the fix; ordering them is not. A backward-compatible change invalidates nothing — a
  defaulted optional parameter or an added return field needs no co-ownership, and demanding it here would
  reject executable plans.
- **Doc steps (`spec-writer`, `work-decomposer`):** every required field of the output contract is filled; the artifact is consistent with its inputs (does the brief cover every AC and match the research map's real paths? does the slice DAG obey file-disjoint + hotspot-serialization rules, and does every AC map to a slice?). For `work-decomposer`, do not approve unless the supplied `plan/slices.json` is a top-level object with array-valued `slices` (the exact seedable shape `{ "slices": [...] }`); inspect only the supplied artifact, not a broader plan schema. Coverage is not enough: where more than one slice exists, a slice claiming the entire acceptance set, or claiming paths spanning every module in its lane, is a BLOCKER — that plan reviews clean and fails later as "N categories missing", which is a scope report rather than a defect report. For class-wide subjects, the brief must include the finite implementation matrix (per §"Class-wide completeness") — a class-wide spec that lacks it is a BLOCKER. Also cross-check the plan against its own order: a slice whose `test_plan` makes a claim that **the landing of a later slice's owned path would invalidate** is a BLOCKER, naming both slices and the path — such a plan cannot be executed in the order it declares, because the earlier slice's suite must fail once the later slice lands and `paths` freeze at seeding. The test is whether the claim survives that path existing, not whether it is phrased negatively: a claim written against process-global state (an import cache, a registry, a singleton) is invalidated as soon as a sibling's tests are collected in the same process, so it blocks; a dependency-direction invariant proven statically over the source or inside an isolated child process stays true afterwards, so it is valid and must **not** be blocked even though it says a later-owned path is unreachable. If the `test_plan` does not name which form it uses, that omission is the finding. File-disjointness does not catch any of this; the two slices share no path. It is decidable from the plan alone, so check it here rather than discovering it at the slice that fails.
