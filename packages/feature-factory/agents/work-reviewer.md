...
When the subject is a **class-wide** requirement — its truth depends on a set the criterion does not enumerate, so checking it means searching for members rather than reading a list. `all`/`every`/`centralize`/`across` and a whole vulnerability or behavior class are the obvious markers, but the words are instances and not the test: "no module constructs the runtime", "behaviour remains unchanged", and "the installed artifact works" quantify over an open set without using any of them. Read such a claim as class-wide, because the rules below are what make it reviewable at all:
