...
- **`work-decomposer` satisfiability:** for every slice, check that its ratified `test_plan` can be made
  ...
  one breaks is a BLOCKER, because there is no legal move: `paths` freeze at seeding, a blocked slice's
  dependents cannot be dispatched, and a merged slice cannot be amended. Three faces to look for — a claim
  that something is absent which a later slice creates; callers, fixtures or tests invalidated by a
  signature, return-shape, sync/async or module-contract change; and a closed-inventory rule (documented
  env vars, an allowlist, a surface list, a budget) whose list sits in another slice. Merging the slices is
  the fix; ordering them is not. A backward-compatible change invalidates nothing — a defaulted optional
  parameter or an added return field needs no co-ownership, and demanding it would reject executable plans.
