...
export function resolveSpawnExecutable(argv0, options = {}) {
  ...
  const candidates = direct ? locations : locations.flatMap((location) => {
    const exact = path.join(location, argv0);
    return windows && !path.extname(argv0) ? [exact, `${exact}.com`, `${exact}.exe`] : [exact];
  });
