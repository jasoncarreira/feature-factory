...
describe("ceiling — scope cannot grow without editing this file", () => {
  ...
  it("keeps the production surface small enough to read in one sitting", () => {
    ...
    // Issue #357 adds the audited operator grant. Issue #359 authorizes up to 5950 lines to make its
    // snapshot revocation recoverable across manifest failure and process death; the exact landing is recorded.
    assert.equal(total, 5950, "recoverable retry-grant transactions land at 5950 production lines");
    ...
    // for operator-authorized retry extension. Issue #359 authorizes 5950 for crash-safe grant recovery.
    // Nothing was trimmed or padded to fit either ledger.
    assert.ok(total <= 5950, `production source is ${total} lines; the issue #359 tripwire is 5950`);
  });
