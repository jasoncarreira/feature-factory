...
const RUN_ORCHESTRATOR = {
  ...
  prompt: [
    ...
    "Use this exact gate artifact map: Story gate `story` -> `.factory/$R/artifacts/story.md`; Brief gate `brief` -> `.factory/$R/artifacts/technical-brief.md`; Pre-PR gate `pre_pr` -> `.factory/$R/gates/pre_pr.md`. Gate artifacts live under the run directory because it is gitignored; writing them to the repository root dirties the integration worktree and parks the run at post-merge verify.",
    "Before every Gate 3 presentation, write or refresh `.factory/$R/gates/pre_pr.md` with the current validator verdict when applicable, the acceptance-criterion/test table, the feature-branch diff and PR-base summary, migration and flag callouts, and remaining risks. Then open Gate 3 with `factory gate \"$R\" pre_pr pending --artifact \".factory/$R/gates/pre_pr.md\" --repo \"$RUN_REPO\"`.",
    "For an interactive background session, an orderly pending-gate park is complete only after you await every in-flight specialized task call and stop heartbeat calls; select `ARTIFACT` from the exact map and verify it exists, refreshing `.factory/$R/gates/pre_pr.md` before Pre-PR; persist the pending gate with `factory gate \"$R\" \"$GATE\" pending --artifact \"$ARTIFACT\" --repo \"$RUN_REPO\"`; directly verify the artifact still exists, the manifest records the named gate pending with `ARTIFACT`, and qualified status reports it pending; release exactly with `factory lock \"$R\" release --session \"$SESSION_ID\" --repo \"$RUN_REPO\"`; and rerun qualified status to verify that session no longer holds the lock.",
