...
const OPERATING_CONTRACTS = [
  ...
  ["publishing identity is active and retained raw while publish remains unconsumed", (text) => /`resolve`, `verify`, and `publishing_identity` are consumed now\. Configured `publish` remains unconsumed and is not invoked\.[\s\S]*retain the raw validated string exactly as parsed, without trimming, normalization, case-folding, or reserialization[\s\S]*Active in every mode at exactly three guards; absent config preserves existing behavior/u.test(text), "`resolve`, `verify`, and `publishing_identity` are consumed now."],
  ["effective push is package-owned rather than configured publish", (text) => text.includes("Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."), "Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."],
