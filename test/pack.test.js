...
const SKILL_CONTRACTS = [
  ...
  ["the infra streak is ephemeral and parks on the second failure", (text) => /Start it at zero in every new driver invocation,\s+including after resume; never write it to `run\.json` or any artifact[\s\S]*On the second consecutive confirmed failure for the same key[\s\S]*Take the common\s+infrastructure-park sequence above/u.test(text), "never write it to `run.json` or any artifact"],
];
