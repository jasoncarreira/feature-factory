...
const LEGACY_CONFIG_SCHEMA = CONFIG_SCHEMA.replace('  "pr_draft": true,\n', "");
...
const ROOT_README_CONTRACTS = [
  ["the four-required plus three-optional schema", (text) => text.includes(LEGACY_CONFIG_SCHEMA), LEGACY_CONFIG_SCHEMA],
  ["timeouts validate and default independently", (text) => /Both timeouts are positive safe integers\. `bootstrap_timeout_ms` requires `bootstrap`\.[\s\S]*Each omitted timeout independently defaults to `900000`; neither shares the other's budget/u.test(text), "Both timeouts are positive safe integers. `bootstrap_timeout_ms` requires `bootstrap`."],
...
const OPERATING_CONTRACTS = [
  ["the four-required plus three-optional schema", (text) => text.includes(LEGACY_CONFIG_SCHEMA), LEGACY_CONFIG_SCHEMA],
  ["timeouts validate and default independently", (text) => /Both timeouts must be positive safe integers\. `bootstrap_timeout_ms`\s+requires `bootstrap`\. Each omitted timeout independently defaults to `900000` milliseconds and neither\s+shares the other's budget/u.test(text), "Both timeouts must be positive safe integers. `bootstrap_timeout_ms`"],
  ["config validation uses runtime first-match order", (text) => text.includes("Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; then missing or invalid required entries."), "Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; then missing or invalid required entries."],
