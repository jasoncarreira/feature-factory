...
const SKILL_CONTRACTS = [
  ["the four-required plus four-optional schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["commands, identity, and optional entries are typed", (text) => /three required own properties `resolve`, `verify`, and `publish`,\s+plus only the optional own properties `pr_draft`, `verify_timeout_ms`, `bootstrap`, and\s+`bootstrap_timeout_ms`[\s\S]*`resolve`, `verify`, `publish`, and `bootstrap` are command strings[\s\S]*There is no `publishing_identity` key: the account a run publishes as is a\s+property of the environment it runs in/u.test(text), "There is no `publishing_identity` key"],
  ...
  ["resolve, verify, and publishing identity are consumed while publish remains unconsumed", (text) => /`resolve` and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the\s+guards below\. Configured `publish` remains unconsumed and is not invoked\./u.test(text)],
...
const IDENTITY_README_CONTRACTS = [
  ...
  ["all modes use exactly the three identity boundaries and absence preserves behavior", (text) => /every mode checks it at exactly three boundaries:[\s\S]*verified post-lock ownership[\s\S]*explicit resume is verified running[\s\S]*before reconciliation or other work; immediately before `git push`, after\s+effective push-target equality; and immediately before `gh pr create`, after the push is known\s+successful[\s\S]*Only a manifest written before 0\.8\.0, which can\s+report `null`, skips all three guards; an absent config does not affect them/u.test(text), "Only a manifest written before 0.8.0, which can"],
...
const README_CONTRACTS = [
  ["the four-required plus four-optional schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["commands, identity, and optional entries are typed", (text) => /three required properties and four optional properties: `pr_draft`, `verify_timeout_ms`,\s+`bootstrap`, and `bootstrap_timeout_ms`[\s\S]*`resolve`, `verify`, `publish`, and a present `bootstrap` are non-empty command\s+strings[\s\S]*There is no `publishing_identity` key, and a file carrying one is malformed because the\s+optional set is closed/u.test(text), "There is no `publishing_identity` key, and a file carrying one is malformed"],
...
const OPERATING_CONTRACTS = [
  ...
  ["publishing identity is compared from the run while publish remains unconsumed", (text) => /`resolve`, `bootstrap`, and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the publication guards\. Configured `publish` remains unconsumed and is not invoked\./u.test(text) && /read the value `status` reports for the run[\s\S]*Absent at init refuses before any sandbox exists/u.test(text), "Absent at init refuses before any sandbox exists"],
  ["effective push is package-owned rather than configured publish", (text) => text.includes("Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."), "Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."],
  ["the three operating boundaries preserve the publication surface", (text) => /immediately after verified post-lock ownership[\s\S]*explicit resume is verified running[\s\S]*before reconciliation or other work;\s+immediately before `git push`, after effective push-target equality; and immediately before\s+`gh pr create`, after the push is known successful[\s\S]*`publishing_identity` is a recorded run field reported by `status`, resolved by `init` from a flag or the environment\. The independent `factory effective-push` command adds no state or flag\.[\s\S]*Existing push, `gh pr create`, `factory pr`, Gate 3, merge, and approval semantics remain unchanged/u.test(text), "Publishing identity is checked immediately after verified post-lock ownership, or immediately after an"],
  ["stale push-target deferrals are absent", (text) => !/#224|push-target migration is deferred|only `publish` remains deferred/u.test(text)],
