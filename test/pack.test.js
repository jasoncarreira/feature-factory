...
const SKILL_CONTRACTS = [
  ...
  ["commands, identity, and optional entries are typed", (text) => /two required own properties `resolve` and `verify`,\s+plus only the optional own properties `publish`, `pr_draft`, `verify_timeout_ms`, `bootstrap`, and\s+`bootstrap_timeout_ms`[\s\S]*`resolve`, `verify`, `publish`, and `bootstrap` are command strings[\s\S]*There is no `publishing_identity` key: the account a run publishes as is a\s+property of the environment it runs in/u.test(text), "There is no `publishing_identity` key"],
