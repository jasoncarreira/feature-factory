...
const SKILL_CONTRACTS = [
  ...
  ["stdout is the unchanged payload with a canonical run id", (text) => /stdout itself is `ISSUE_PAYLOAD`[\s\S]*canonical top-level string field named `run_id`[\s\S]*exact same stdout bytes unchanged to `story-reader`/u.test(text)],
  ["only an absent config uses compatibility behavior", (text) => /Only an absent `\$O\/\.factory\/config\.json` selects the compatibility path/u.test(text)],
  ["malformed config refuses closed", (text) => text.includes("invalid factory config: .factory/config.json; no session or run created.")],
  ["malformed payload refuses closed", (text) => text.includes("factory config entry 'resolve' returned malformed payload; no session or run created.")],
  ["an observed status is reported without fallback", (text) => text.includes("factory config entry 'resolve' failed with exit status <status>; no session or run created.")],
  ["an unavailable status is reported without fallback", (text) => text.includes("factory config entry 'resolve' failed; exit status unavailable; no session or run created.")],
  ["commands and credentials are never disclosed", (text) => /Never print, quote,\s+reproduce, log, or persist the configured command string, an expanded or resolved command line,\s+credentials, or shell\/tool diagnostics/u.test(text)],
...
const README_CONTRACTS = [
  ...
  ["present invalid config refuses and only absence falls back", (text) => /present invalid,[\s\S]*config refuses closed\. Only an absent file selects the\s+existing GitHub issue behavior/u.test(text)],
