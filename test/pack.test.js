...
const SKILL_CONTRACTS = [
  ...
  ["commands, identity, and optional entries are typed", (text) => /four required own properties `resolve`, `verify`, `publish`, and\s+`publishing_identity`, plus only the optional own properties `verify_timeout_ms`, `bootstrap`, and\s+`bootstrap_timeout_ms`[\s\S]*`resolve`, `verify`, `publish`, and `bootstrap` are command strings[\s\S]*`publishing_identity` is\s+a static non-empty publishing account name/u.test(text), "`publishing_identity`, plus only the optional own properties `verify_timeout_ms`, `bootstrap`, and"],
...
const README_CONTRACTS = [
  ["the four-required plus three-optional schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["commands, identity, and optional entries are typed", (text) => /four required properties and three optional properties: `verify_timeout_ms`, `bootstrap`,\s+and `bootstrap_timeout_ms`[\s\S]*`resolve`, `verify`, `publish`, and a present `bootstrap` are non-empty command\s+strings[\s\S]*`publishing_identity` is a static non-empty account name/u.test(text), "The root has four required properties and three optional properties: `verify_timeout_ms`, `bootstrap`,"],
