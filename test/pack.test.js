...
const SKILL_CONTRACTS = [
  ...
  ["publish replaces only PR creation after protected push", (text) => /Configured `publish`, when present, replaces only the driver's `gh pr create` in Step 6;\s+the factory-owned exact push and post-push identity guard remain unchanged\./u.test(text)],
  ...
  ["configured publish receives bounded inputs after protected push", (text) => /Exact configured string as one shell step in `RUN_REPO` cwd[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*in place of only `gh pr create`, after the factory-owned exact push and post-push identity guard[\s\S]*Active at the three mandatory guards below/u.test(text)],
  ...
  ["the infra streak is ephemeral and parks on the second failure", (text) => /Start it at zero in every new driver invocation,\s+including after resume; never write it to `run\.json` or any artifact[\s\S]*On the second consecutive confirmed failure for the same key[\s\S]*preserve the same\s+durable attempt[\s\S]*top-level needs-human parked-stop procedure/u.test(text), "never write it to `run.json` or any artifact"],
];
...
const README_CONTRACTS = [
  ...
  ["specialist infrastructure failures preserve attempt budget", (text) => /confirmed host-origin HTTP availability or transport failure does not spend a specialist attempt[\s\S]*retries once at the same persisted attempt[\s\S]*(?:recover the same child\/session|recover the same child)[\s\S]*second consecutive failure for the same role and\s+subject parks the run[\s\S]*(?:memory-only|counter is\s+memory-only)/u.test(text), "does not spend a specialist attempt"],
  ["optional publish replaces only PR creation with bounded inputs", (text) => /Configured `publish` is optional[\s\S]*replaces only (?:PR creation|`gh pr create` in Step 6)[\s\S]*factory-owned exact push and post-push identity guard[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*absolute HTTPS URL[\s\S]*parks without fallback/u.test(text)],
...
const OPERATING_CONTRACTS = [
  ...
  ["bootstrap absence is an exact no-op", (text) => text.includes("When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."), "When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."],
  ["bootstrap exclusions preserve established behavior", (text) => /Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation\. Existing resolver, verify, configured-publish, effective-push, push, PR, and Gate 3 behavior is unchanged\./u.test(text), "Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation."],
  ["specialist infrastructure errors preserve attempt budget", (text) => /Only a host-origin invocation error with a closed availability status or transport code qualifies[\s\S]*first qualifying failure keeps the current attempt[\s\S]*same-session recovery[\s\S]*second consecutive failure for\s+the same role and subject parks the run[\s\S]*memory-only and resets with the active\s+driver[\s\S]*never duplicate a\s+possibly started child or classify words found in model output/u.test(text), "Only a host-origin invocation error with a closed availability status or transport code qualifies."],
  ["bootstrap exclusions preserve established behavior", (text) => /Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation\. Bootstrap does not mediate configured publication, effective-push, push, PR, or Gate 3 behavior\./u.test(text), "Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation."],
  ...
  ["publishing identity is compared and optional publish replaces only PR creation", (text) => /`resolve`, `bootstrap`, and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the publication guards\. Configured `publish` is optional and replaces only PR creation after the factory-owned exact push and post-push identity guard\./u.test(text) && /read the value `status` reports for the run[\s\S]*Absent at init refuses before any sandbox exists/u.test(text), "Absent at init refuses before any sandbox exists"],
  ["configured publish has bounded input and indeterminate-failure semantics", (text) => /`publish` \| Optional exact string run as one shell step in `RUN_REPO`[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*absolute HTTPS URL[\s\S]*every other result is indeterminate and parks without fallback or `factory pr`[\s\S]*Replaces only `gh pr create`, after the factory-owned exact push and post-push identity guard/u.test(text)],
  ["effective push is package-owned rather than configured publish", (text) => text.includes("Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."), "Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."],
