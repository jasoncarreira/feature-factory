...
const SKILL_CONTRACTS = [
  ["the declared repository-config schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["commands, identity, and optional entries are typed", (text) => /two required own properties `resolve` and `verify`,\s+plus only the optional own properties `publish`, `pr_draft`, `verify_timeout_ms`, `bootstrap`,\s+`bootstrap_timeout_ms`, and `max_retries`[\s\S]*`resolve`, `verify`, `publish`, and `bootstrap` are command strings[\s\S]*There is no `publishing_identity` key: the account a run publishes as is a\s+property of the environment it runs in/u.test(text), "There is no `publishing_identity` key"],
  ["bootstrap and verify have independent defaults", (text) => /`verify_timeout_ms` and `bootstrap_timeout_ms` each independently default to `900000` milliseconds;\s+neither timeout shares or consumes the other's budget/u.test(text), "`verify_timeout_ms` and `bootstrap_timeout_ms` each independently default to `900000` milliseconds;"],
  ["config validation uses runtime first-match order", (text) => text.includes("Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."), "Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."],
  ...
  ["bootstrap manifest evidence is paired", (text) => text.includes("A successful configured attempt stores the exact command in `bootstrap_command` and the numeric result in paired `bootstrap_exit`."), "A successful configured attempt stores the exact command in `bootstrap_command` and the numeric result in paired `bootstrap_exit`."],
  ...
  ["configured resume binds bytes and owner writes", (text) => /For configured order 7, the CLI binds the exact raw `run\.json` bytes[\s\S]*Every factory-mediated claim, force-steal, refresh, and release holds `run-json\.lock`[\s\S]*A clean zero records the command and exit `0`[\s\S]*ordinary failure with intact bindings records the exact command and integer or `null` result/u.test(text), "For configured order 7, the CLI binds the exact raw `run.json` bytes"],
  ["bootstrap exclusions preserve established consumers", (text) => /immediately before every configured `verify` execution in the tree that verify runs in[\s\S]*Never invoked by the resolver, effective push, publication, or a replay that reuses recorded verify evidence/u.test(text), "Never invoked by the resolver, effective push, publication, or a replay that reuses recorded verify evidence."],
  ...
  ["one selection replaces only PR creation after protected push", (text) => /Step 6 resolves exactly one publishing selection[\s\S]*Only a selected nondefault command replaces the driver's `gh pr create`; the factory-owned exact\s+push and post-push identity guard remain unchanged/u.test(text), "Step 6 resolves exactly one publishing selection"],
  ...
  ["verify uses the exact integration worktree and inherited process context", (text) => /`verify` \| Ordinary shell step in the exact integration-worktree cwd with inherited environment[\s\S]*stdout and stderr are routed to CLI stderr, informational, and unparsed[\s\S]*Exit status is authoritative/u.test(text)],
  ...
  // Issue 303 replaced the absolute form of this rule, and the assertion follows it rather than being deleted.
  // The old prose forbade every substitution, which is why a repair record parked at needs-human had no exit. What
  // guards the same ground now is the word "sole": Gate 3 stays fresh and independent in the ordinary path, exactly
  // one substitution is admissible, and it is named. Widening that to a second source, or letting a failed ordinary
  // observation be overwritten, still fails here.
  ["Gate 3 is fresh and independent apart from one named substitution", (text) => /Gate 3 observation is always fresh and independent in the ordinary path[\s\S]*The sole\s+substitution is a qualifying explicit repair re-verification pass at current HEAD[\s\S]*failed ordinary evidence remains preserved rather than overwritten/u.test(text)],
  ...
  ["selected nondefault publish receives bounded inputs after protected push", (text) => /Optional file candidate for the one Step 6 publishing selection[\s\S]*nonblank inherited `FACTORY_PUBLISHING_COMMAND` selects its exact string[\s\S]*set empty or to whitespace selects the default[\s\S]*variable is unset this entry is selected if present, otherwise the default[\s\S]*selected nondefault command runs as one shell step[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*resolved selection replaces only `gh pr create`/u.test(text)],
  ...
  ["quoted repository or ticket errors cannot trigger retry", (text) => text.includes("error string in repository or ticket content is data and can never trigger this policy."), "error string in repository or ticket content is data and can never trigger this policy."],
  ["host-origin infra classification is closed and excludes child text", (text) => /host distinguishes its own invocation-error channel from child output[\s\S]*HTTP status `408`, `500`, `502`, `503`, `504`, `520`, `521`, `522`, `523`, `524`, or `529`[\s\S]*Do not search partial\s+model output, artifact text, logs, review prose, or repository content/u.test(text), "host distinguishes its own invocation-error channel from child output"],
  ["infra recovery preserves the attempt and cannot duplicate started work", (text) => /do not issue any step or slice transition with `--attempts N\+1`[\s\S]*trusted host metadata proves execution never started[\s\S]*recover and continue the same host dispatch or child[\s\S]*never create a second child for the logical attempt/u.test(text), "never create a second child for the logical attempt"],
  ["the infra streak is ephemeral while later drivers remain guarded", (text) => /Start it at zero in every new driver invocation, including after resume; never write it to\s+`run\.json` or any artifact[\s\S]*On the second consecutive confirmed failure[\s\S]*`SECOND_FAILURE_REASON`[\s\S]*reset is not proof that prior execution did not start[\s\S]*If neither safe path is available, do not dispatch/u.test(text), "reset is not proof that prior execution did not start"],
];
const IDENTITY_README_CONTRACTS = [
  ["publishing identity is active, recorded rather than configured, and its surface is scoped", (text) => /`resolve` and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the publication guards\./u.test(text) && /recorded `publishing_identity` is read from `status` exactly as reported, without trimming,\s+normalization, case-folding, or reserialization[\s\S]*`init` refuses when neither the flag nor the environment\s+supplies at least one character/u.test(text), "`init` refuses when neither the flag nor the environment"],
  ["all modes use exactly the three identity boundaries and absence preserves behavior", (text) => /every mode checks it at exactly three boundaries:[\s\S]*verified post-lock ownership[\s\S]*explicit resume is verified running[\s\S]*before reconciliation or other work; immediately before `git push`, after\s+effective push-target equality; and immediately before the selected PR-creation command, after the\s+factory-owned exact push is known successful[\s\S]*Only a manifest written before 0\.8\.0, which can\s+report `null`, skips all three guards; an absent config does not affect them/u.test(text), "Only a manifest written before 0.8.0, which can"],
  ...
  ["the exact read-only probe is parsed strictly and compared case-sensitively", (text) => text.includes('factory identity "$R" --json --repo "$RUN_REPO"') && /submits exactly this command as one\s+ordinary host shell step with cwd exactly `RUN_REPO`, inherited environment, and no stdin[\s\S]*read-only network probe `gh api --method GET \/user --jq \.login` itself, with separate\s+stdout and stderr pipes[\s\S]*probe's stdout bytes, stderr bytes, and numeric status are parsed strictly[\s\S]*exactly one ASCII GitHub login plus one LF[\s\S]*compared exactly and case-sensitively[\s\S]*`gh auth status`\s+does not prove the publishing identity/u.test(text), "The probe's stdout bytes, stderr bytes, and numeric status are parsed strictly."],
  ...
  ["identity verification is enforcement while credential setup is instruction", (text) => /Publishing-identity verification is enforcement because it prevents false-green or wrong-account\s+publication\. Credential provisioning and helper setup are instruction only[\s\S]*Existing push,\s+`factory pr`, Gate 3, merge, and approval semantics remain unchanged/u.test(text), "Publishing-identity verification is enforcement because it prevents false-green or wrong-account"],
];
const README_CONTRACTS = [
  ["the declared repository-config schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["commands, identity, and optional entries are typed", (text) => /two required properties, `resolve` and `verify`, and six optional properties: `publish`,\s+`pr_draft`, `verify_timeout_ms`, `bootstrap`, `bootstrap_timeout_ms`, and `max_retries`[\s\S]*Required commands and any present\s+`publish` or `bootstrap` are non-empty strings[\s\S]*There is no `publishing_identity` key, and a file carrying one is malformed because the\s+optional set is closed/u.test(text), "There is no `publishing_identity` key, and a file carrying one is malformed"],
  ["timeouts validate and default independently", (text) => /A present `pr_draft` must be a JSON boolean and omission means `true`\. Both timeouts are\s+positive safe integers\. `bootstrap_timeout_ms` requires `bootstrap`\.[\s\S]*Each omitted timeout independently defaults to `900000`; neither shares the other's budget/u.test(text), "A present `pr_draft` must be a JSON boolean and omission means `true`. Both timeouts are"],
  ["config validation uses runtime first-match order", (text) => text.includes("Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."), "Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."],
  ["bootstrap execution, routing, and timeout are exact", (text) => /Only the CLI executes configured `bootstrap`: once during fresh init after clone, containment, and PR-base observation but before manifest publication, and again on every explicit resume while the run remains parked\.[\s\S]*exact configured string runs unchanged with `shell: true`, inherited environment and stdin, cwd exactly the selected sandbox, and child stdout and stderr both routed to CLI stderr[\s\S]*independent `900000` millisecond default/u.test(text), "Only the CLI executes configured `bootstrap`: once during fresh init after clone, containment, and PR-base observation but before manifest publication"],
  ["bootstrap cleanliness and evidence are exact", (text) => /checks tracked worktree and index paths only and ignores untracked dependency output[\s\S]*clean numeric zero stores paired `bootstrap_command` and `bootstrap_exit` manifest evidence[\s\S]*Ordinary transitions preserve the pair, and status output does not expose it/u.test(text), "After every attempt, the CLI checks tracked worktree and index paths only and ignores untracked dependency output."],
  ["fresh and resumed bootstrap outcomes are exact", (text) => /failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run\.json` absent[\s\S]*Configured resume first binds exact raw manifest bytes[\s\S]*Ordinary failure with intact bindings records integer or `null` evidence[\s\S]*Byte mutation or owner loss preserves current state and ownership, records no evidence, and does not unpark[\s\S]*serialize with resume publication through `run-json\.lock`/u.test(text), "A failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run.json` absent."],
  ["bootstrap absence is an exact no-op", (text) => text.includes("When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."), "When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."],
  ["bootstrap exclusions preserve established behavior", (text) => /Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation\. Bootstrap does not mediate configured publication, effective-push, push, PR, or Gate 3 behavior\./u.test(text), "Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation."],
  ["resolve and verify are consumed", (text) => /`resolve` and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the/u.test(text)],
  ["specialist infrastructure failures preserve attempt budget and later-driver safety", (text) => /confirmed host-origin availability or transport failure does not spend a specialist attempt[\s\S]*same-attempt recovery for the canonical role and subject[\s\S]*Unbudgeted research and design create no\s+attempt[\s\S]*resume and reset never prove that prior work did not start[\s\S]*recover or prove the\s+same prior invocation safe before dispatch/u.test(text), "resume and reset never prove that prior work did not start"],
  ["resolved publication applies all four precedence branches", (text) => /Step 6 resolves one selection: a nonblank inherited `FACTORY_PUBLISHING_COMMAND` selects its exact string[\s\S]*variable set blank or whitespace selects the default[\s\S]*when it is unset, configured `publish` wins if present; otherwise the default wins[\s\S]*Only a selected nondefault command replaces (?:PR creation|`gh pr create`)[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*selected publishing command outcome indeterminate[\s\S]*no fallback/u.test(text), "Step 6 resolves one selection:"],
...
const ROOT_README_CONTRACTS = [
  ["the declared repository-config schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["timeouts validate and default independently", (text) => /A present `pr_draft` must be a JSON boolean and omission means `true`\.\s+Both timeouts are positive safe integers\. `bootstrap_timeout_ms` requires `bootstrap`\.[\s\S]*Each omitted timeout independently defaults to `900000`; neither shares the other's budget/u.test(text), "Both timeouts are positive safe integers. `bootstrap_timeout_ms` requires `bootstrap`."],
  ["config validation uses runtime first-match order", (text) => text.includes("Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."), "Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."],
  ["bootstrap execution, routing, and timeout are exact", (text) => /Only the CLI executes configured `bootstrap`: once during fresh init after clone, containment, and PR-base observation but before manifest publication, and again on every explicit resume while the run remains parked\.[\s\S]*exact configured string runs unchanged with `shell: true`, inherited environment and stdin, cwd exactly the selected sandbox, and child stdout and stderr both routed to CLI stderr[\s\S]*independent `900000` millisecond default/u.test(text), "Only the CLI executes configured `bootstrap`: once during fresh init after clone, containment, and PR-base observation but before manifest publication"],
  ["bootstrap cleanliness and evidence are exact", (text) => /checks tracked worktree and index paths only and ignores untracked dependency output[\s\S]*clean numeric zero stores paired `bootstrap_command` and `bootstrap_exit` manifest evidence[\s\S]*Ordinary transitions preserve the pair, and status output does not expose it/u.test(text), "After every attempt, the CLI checks tracked worktree and index paths only and ignores untracked dependency output."],
  ["fresh and resumed bootstrap outcomes are exact", (text) => /failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run\.json` absent[\s\S]*Configured resume first binds exact raw manifest bytes[\s\S]*Ordinary failure with intact bindings records integer or `null` evidence[\s\S]*Byte mutation or owner loss preserves current state and ownership, records no evidence, and does not unpark[\s\S]*serialize with resume publication through `run-json\.lock`/u.test(text), "A failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run.json` absent."],
...
const OPERATING_CONTRACTS = [
  ["the declared repository-config schema", (text) => text.includes(CONFIG_SCHEMA), CONFIG_SCHEMA],
  ["timeouts validate and default independently", (text) => /A present `pr_draft` must be a JSON boolean and omission means `true`\.\s+Both timeouts must be positive safe integers\. `bootstrap_timeout_ms`\s+requires `bootstrap`\. Each omitted timeout independently defaults to `900000` milliseconds and neither\s+shares the other's budget/u.test(text), "Both timeouts must be positive safe integers. `bootstrap_timeout_ms`"],
  ["config validation uses runtime first-match order", (text) => text.includes("Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."), "Validation refuses the first matching defect in this order: unreadable or invalid JSON, a non-object root, or unknown keys; invalid `pr_draft`; invalid `bootstrap`; `bootstrap_timeout_ms` without `bootstrap`; invalid `bootstrap_timeout_ms`; invalid `verify_timeout_ms`; missing or invalid required entries; invalid `publish`; then invalid `max_retries`."],
  ["bootstrap execution and routing are exact", (text) => /`bootstrap` \| The exact string runs unchanged with `shell: true`, cwd exactly the selected sandbox, inherited environment and stdin, and child stdout and stderr both routed to CLI stderr[\s\S]*again on every explicit resume while parked\. No retry/u.test(text), "| `bootstrap` | The exact string runs unchanged with `shell: true`, cwd exactly the selected sandbox"],
  ["bootstrap cleanliness and evidence are exact", (text) => /Bootstrap cleanliness checks tracked worktree and index paths only, so untracked dependency installation is allowed[\s\S]*paired top-level `bootstrap_command` and `bootstrap_exit` evidence[\s\S]*status response shape stays unchanged/u.test(text), "Bootstrap cleanliness checks tracked worktree and index paths only, so untracked dependency installation is allowed."],
  ["fresh and resumed bootstrap outcomes are exact", (text) => /failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run\.json` absent[\s\S]*Configured resume binds exact raw manifest bytes[\s\S]*Ordinary failure with intact bindings records integer or `null`[\s\S]*Manifest-byte or owner binding loss preserves current state and owner, records no evidence, and does not unpark[\s\S]*serialize with manifest publication through `run-json\.lock`/u.test(text), "A failed, timed-out, dirty, or unobservable fresh init emits no JSON stdout, retains the deterministic sandbox, and leaves `run.json` absent."],
  ["bootstrap absence is an exact no-op", (text) => text.includes("When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."), "When both bootstrap keys are absent, init and resume are exact no-ops for bootstrap: no execution, manifest fields, output, or response-shape change."],
  ["specialist infrastructure errors use the closed classifier and later-driver guard", (text) => /Only a host-origin invocation error qualifies[\s\S]*no complete response exists[\s\S]*`socket closed unexpectedly`[\s\S]*`AI_APICallError: Service Unavailable \(503\)`[\s\S]*same canonical role and subject[\s\S]*memory-only and resets with the active\s+driver, but the later-driver guard still requires recovery or no-start proof before dispatch/u.test(text), "later-driver guard still requires recovery or no-start proof before dispatch"],
  ["bootstrap exclusions preserve established behavior", (text) => /Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation\. Bootstrap does not mediate configured publication, effective-push, push, PR, or Gate 3 behavior\./u.test(text), "Bootstrap never runs during resolver intake, merge verification or replay, direct repository verification, slice observation, Gate 3, effective push, configured publication, push, or PR creation."],
  ...
  ["publishing identity is compared and one selection controls PR creation", (text) => /`resolve`, `bootstrap`, and `verify` are consumed now, and the run's recorded `publishing_identity` is compared at the publication guards\. Step 6 resolves one selection[\s\S]*nonblank inherited `FACTORY_PUBLISHING_COMMAND` selects its exact string[\s\S]*set blank or whitespace selects the default[\s\S]*when it is unset, configured `publish` wins if present; otherwise the default wins[\s\S]*Only a selected nondefault command replaces PR creation[\s\S]*factory-owned exact push and post-push identity guard/u.test(text) && /read the value `status` reports for the run[\s\S]*Absent at init refuses before any sandbox exists/u.test(text), "Step 6 resolves one selection:"],
  ["selected publish has bounded input and exact indeterminate reason", (text) => /`publish` \| Optional candidate string run as one shell step in `RUN_REPO`[\s\S]*`PR_BASE`, `FEATURE_BRANCH`, `PR_DRAFT`, `PR_TITLE`, and absolute `PR_BODY_FILE`[\s\S]*absolute HTTPS URL[\s\S]*selected publishing command outcome indeterminate; re-observe whether the pull request exists before retry[\s\S]*Selected only by the Step 6 environment\/file\/default precedence/u.test(text)],
  ["effective push is package-owned rather than configured publish", (text) => text.includes("Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."), "Effective push-target capture and comparison are active through the package-owned `factory effective-push` command; they are not deferred to configured `publish`."],
  ["the three operating boundaries preserve the publication surface", (text) => /immediately after verified post-lock ownership[\s\S]*explicit resume is verified running[\s\S]*before reconciliation or other work;\s+immediately before `git push`, after effective push-target equality; and immediately before the selected\s+PR-creation command, after the factory-owned exact push is known successful[\s\S]*`publishing_identity` is a recorded run field reported by `status`, resolved by `init` from a flag or the environment\. The independent `factory effective-push` command adds no state or flag\.[\s\S]*Existing push, `factory pr`, Gate 3, merge, and approval semantics remain unchanged/u.test(text), "Publishing identity is checked immediately after verified post-lock ownership, or immediately after an"],
  ["stale push-target deferrals are absent", (text) => !/#224|push-target migration is deferred|only `publish` remains deferred/u.test(text)],
  ...
  ["the operator probe is exact, direct, strict, and case-sensitive", (text) => text.includes('factory identity "$R" --json --repo "$RUN_REPO"') && /submits exactly this command as one\s+ordinary host shell step with cwd exactly `RUN_REPO`, inherited environment, and no stdin[\s\S]*read-only network probe `gh api --method GET \/user --jq \.login` itself, with separate\s+stdout and stderr pipes[\s\S]*CLI reads the probe's exact stdout bytes, exact stderr bytes, and numeric status[\s\S]*exactly one ASCII login matching[\s\S]*followed by exactly one LF byte[\s\S]*compare the raw declared and observed strings exactly and case-sensitively[\s\S]*`gh auth status`; machine-active authentication is not\s+proof/u.test(text), "The CLI reads the probe's exact stdout bytes, exact stderr bytes, and numeric status."],
  ...
  // Both cost most of a day to learn: the stdin hang was diagnosed as a broken tool and an upstream bug was
  // drafted for it, and the turn limits were then rediscovered after a run sat initialized and undriven for
  // three hours. Pinned so the recipe cannot lose them quietly.
  ["an unattended launch closes stdin", (text) => text.includes("**Redirect stdin from `/dev/null`.**")
    && text.includes("it blocks before emitting a byte"), "**Redirect stdin from `/dev/null`.**"],
  ["the autonomous defaults are named as insufficient", (text) => text.includes("**Raise every autonomous limit.**")
    && text.includes("12 assistant turns, 3 continuations, 80,000 tokens and a 30-minute wall"),
    "12 assistant turns, 3 continuations, 80,000 tokens and a 30-minute wall"],
...
const OPENCODE_SKILL_CONTRACTS = [
  ["publish presence is only one selection candidate", (text) => /`publish` was required and invoked nowhere until this release[\s\S]*contributes only the file candidate\s+to the one Step 6 publishing selection[\s\S]*presence alone never executes this entry/u.test(text), "presence alone never executes this entry"],
  ["headless gates use the full parked-stop sequence", (text) => /headless run reached a human gate[\s\S]*quiesce outstanding work, terminalize, attempt the\s+parked control-plane snapshot, release the verified owner, prove the lock absent with null owner[\s\S]*report only\s+`Outcome: retained-lock-error` if release or unlock verification fails/u.test(text), "prove the lock absent with null owner"],
];
const CHANGELOG_CONTRACTS = [
  ["publication selection and fixed reason are recorded", (text) => /Configured publication is optional and real[\s\S]*exact nonblank inherited override[\s\S]*set blank or whitespace override[\s\S]*configured `publish` when\s+present[\s\S]*default when neither source exists[\s\S]*selected publishing command outcome indeterminate; re-observe whether the pull request exists before retry/u.test(text), "selects the default when neither source exists"],
  ["the release records operator recovery, Prime dispatch, validator behavior, and infrastructure safety", (text) => /Post-merge production defects have an explicit operator exit \(#342\)[\s\S]*Prime specialist dispatch is live again \(#345\)[\s\S]*single-slice run skips only the duplicate validator verdict \(#346\)[\s\S]*Specialist transport failures preserve the work-attempt budget \(#349, #350\)/u.test(text), "Post-merge production defects have an explicit operator exit (#342)"],
];
...
describe("what actually ships", () => {
  it("packs every file the entrypoints and the docs require", () => {
    ...
    try {
      ...
      const sourceOpenCodeSkill = readFileSync(join(root, "packages", "opencode-feature-factory", "skills", "feature", "SKILL.md"), "utf8");
      const sourceChangelog = readFileSync(join(root, "CHANGELOG.md"), "utf8");
      ...
      assertContracts(sourceOpenCodeSkill, OPENCODE_SKILL_CONTRACTS, "OpenCode skill");
      assertContracts(sourceChangelog, CHANGELOG_CONTRACTS, "changelog");
