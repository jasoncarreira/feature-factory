...
const SKILL_CONTRACTS = [
  ...
  ["verify has closed classification, bounded unavailable replay, and independent Gate 3", (text) => /Classify it into exactly four outcomes:[\s\S]*`green`[\s\S]*`failed`[\s\S]*`unavailable`[\s\S]*`unknown`[\s\S]*only replay-eligible class[\s\S]*There is no third attempt[\s\S]*Gate 3 remains a fresh independent observation/u.test(text)],
  ["clean exhaustion quiesces and verifies release while unsafe state remains terminal", (text) => /terminalize means terminate the current `factory slice … merged` CLI invocation[\s\S]*does not mean the irreversible\s+factory terminal transition[\s\S]*Await every in-flight specialist task[\s\S]*await every heartbeat already in flight[\s\S]*Outcome: repository-verify-exhausted[\s\S]*Outcome: retained-lock-error[\s\S]*unsafe dirty or moved replay[\s\S]*terminalize/u.test(text)],
...
const README_CONTRACTS = [
  ...
  ["classification, safe replay, and invocation-only exhaustion are documented", (text) => /classified as `green`, `failed`, `unavailable`, or `unknown`[\s\S]*only\s+`unavailable` is replay-eligible[\s\S]*Two clean, unchanged unavailable executions terminate the current CLI and driver invocations, not the\s+durable run[\s\S]*`repository-verify-exhausted`[\s\S]*`retained-lock-error`/u.test(text)],
...
const ROOT_README_CONTRACTS = [
  ...
  ["classification, safe replay, and invocation-only exhaustion are documented", (text) => /classified as `green`, `failed`,\s+`unavailable`, or `unknown`; only `unavailable` may execute again[\s\S]*Two clean, unchanged unavailable executions end only the current merge\/replay CLI invocation and its\s+enclosing driver invocation[\s\S]*`repository-verify-exhausted`[\s\S]*`retained-lock-error`/u.test(text)],
...
const OPERATING_CONTRACTS = [
  ...
  ["closed classification and verify-only bounded replay are documented", (text) => /exactly four classifications: `green`, `failed`,\s+`unavailable`, and `unknown`[\s\S]*Only `unavailable` may execute again[\s\S]*there is no aggregate\s+timer, third attempt[\s\S]*timeout and\s+retry are verify-only/u.test(text)],
