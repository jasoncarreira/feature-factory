...
const SKILL_CONTRACTS = [
  ...
  ["resolve, verify, and publishing identity are consumed while publication remains deferred", (text) => /`resolve`, `verify`, and `publishing_identity` are consumed now\. Only `publish` remains deferred to #224/u.test(text)],
  ...
  ["publication remains deferred while identity enforcement is active", (text) => /push-target migration is deferred to #224[\s\S]*Active at the three mandatory guards below/u.test(text)],
];
const IDENTITY_README_CONTRACTS = [
  ["publishing identity is active, retained raw, and adds no surface", (text) => text.includes("`resolve`, `verify`, and `publishing_identity` are consumed now; only `publish` remains deferred to #224.") && /validated raw `publishing_identity` is retained exactly as parsed, without trimming, normalization,\s+case-folding, or reserialization[\s\S]*adds no config key or syntax, run status, or factory command/u.test(text), "`resolve`, `verify`, and `publishing_identity` are consumed now; only `publish` remains deferred to #224."],
...
const OPERATING_CONTRACTS = [
  ...
  ["publishing identity is active and retained raw while only publish is deferred", (text) => /`resolve`, `verify`, and `publishing_identity` are consumed today;\s+only `publish` remains deferred to #224[\s\S]*retain the raw validated string exactly as parsed, without trimming, normalization, case-folding, or reserialization[\s\S]*Active in every mode at exactly three guards; absent config preserves existing behavior/u.test(text), "`resolve`, `verify`, and `publishing_identity` are consumed today;"],
  ["the three operating boundaries preserve the publication surface", (text) => /immediately after verified post-lock ownership[\s\S]*explicit resume is verified running[\s\S]*before reconciliation or other work;\s+immediately before `git push`, after effective push-target equality; and immediately before\s+`gh pr create`, after the push is known successful[\s\S]*no config key or syntax, run status,\s+or factory command[\s\S]*existing push, `gh pr create`, `factory pr`, Gate 3, merge, and approval semantics\s+remain unchanged/u.test(text), "Publishing identity is checked immediately after verified post-lock ownership, or immediately after an"],
