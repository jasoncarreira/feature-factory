...
function createExecutionFixture(runId, commands = [{ program: "node", args: ["--test", "test/acceptance.test.js"] }, { program: "npm", args: ["run", "check"] }]) {
  ...
  const run = {
    ...
    slices: [{ id: "slice", stack: "backend", depends_on: [], status: "merged", attempts: 1, attempt_reviews: [sliceAttemptReview], evidence_ref: sliceEvidenceRef, evidence_hash: sliceEvidenceHash, review_ref: sliceReviewRef, review_hash: sliceReviewHash, reviewed_commit: head, merge_commit: head }],
    steps: [
