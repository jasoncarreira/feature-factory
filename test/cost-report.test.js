...
describe("cost report domain", () => {
  ...
  it("suppresses mixed-currency totals while retaining component-field compatibility", () => {
    const report = buildCostReport("run-123", { entries: [
      availableEntry({ id: "usd", agent: "same", step: "same", slice_id: "same", cost_total: 0.1, cost_input: 0.04, cost_currency: "USD" }),
      availableEntry({ id: "eur", agent: "same", step: "same", slice_id: "same", cost_total: 0.2, cost_input: 0.05, cost_currency: "EUR" }),
    ] });
    ...
    for (const rollup of [report.totals, report.by_agent.same, report.by_step.same, report.by_slice.same]) {
      ...
      assert.equal(rollup.cost_input, 0.09);
      assert.deepEqual(rollup.missing, ["mixed_currency"]);
    }
  });
  ...
  it("serializes control-safe JSON while preserving decoded raw keys", () => {
    const raw = "group\u0085\u2028x";
