...
- B4.1 reserves optional closed `plan.delivery_envelope` schema v1 and optional closed slice-review `invariant_family_ledger` schema v1. An envelope has exactly one delivery unit per plan slice; each unit declares globally unique lowercase kebab-case family, obligation, and artifact IDs, and every obligation references exactly one family plus one artifact whose index and text bind one existing test-plan entry on that slice. A supplied ledger binds its delivery unit and unique family dispositions to mapped artifact IDs, exact evidence ref/hash, a typed probe/result, the enclosing reviewed commit, and canonical unresolved findings. Supplied records validate now; omission remains compatible until B4.2/B4.4 activation. The admission and review extension results are explicitly `status:"inactive"` with `grants_b4_authority:false`, so neither a valid envelope nor a valid ledger currently admits, approves, routes, or otherwise grants B4 authority.
