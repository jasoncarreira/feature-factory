...
- Every slice-review attempt under a delivery envelope requires a complete closed `invariant_family_ledger`: exactly one disposition per current family, mapped through a current obligation to an artifact, with the enclosing reviewed commit and zero omitted or extra families. Before review, the orchestrator runs each selected exact artifact command only through `feature-factory factory artifact-execute <run-id> <slice-id> <artifact-id> --json`. The create-only checked receipt binds run, slice, attempt, accepted plan hash, reviewed HEAD, artifact ID, exact parsed program/argv, and observed process result. Arbitrary evidence bytes, stale receipts, caller-claimed pass results, failed/skipped dispositions, or unresolved findings cannot grant APPROVE authority.
- Oversized routing requires the exact same-attempt APPROVE work-decomposer review already bound in accepted step authority. The deterministic content-addressed checkpoint manifest binds that review plus exact plan/admission authority, and the parent terminal result is closed to `blocked`, reason `oversized-plan-checkpoint-routing-required`, and exactly its `checkpoint_routing` artifact. Start each entry with `factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json`; Git atomically verifies current exact `main` while no-replace reserving both child identity and parent/checkpoint route. Checkpoint 1 rejects predecessor authority; checkpoint N requires every prior child to have a completed normal PR and requires the immediate predecessor's verified merge commit to contain its PR head and be on current `main`. Child payload/resume authority binds the exact parent, manifest, request, base, predecessor, and reservations; entries run strictly in order as fresh full feature runs with their own integration gate, panels, Gate 3, and one PR.
