...
- Oversized routing requires the exact same-attempt APPROVE work-decomposer review already bound in accepted step authority. The deterministic content-addressed checkpoint manifest binds that review plus exact plan/admission authority, and the parent terminal result is closed to `blocked`, reason `oversized-plan-checkpoint-routing-required`, and exactly its `checkpoint_routing` artifact. Start each entry with `factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json`; Git atomically verifies current exact `main` while reserving both child identity and parent/checkpoint route through `reserved`, `launching`, `launched`, or fail-closed `unknown` states. Exact pre-launch retry and exact existing-child adoption do not duplicate launch; conflicting or indeterminate reservations reject. Checkpoint N requires every prior child to prove all slices merged, a fresh accepted passing checked test-verifier receipt, exact-head validator/security approval, Gate 3 approval, completed PR identity, and a canonical GitHub merged-PR observation whose merge commit contains that head and is on current `main`. Local-only, stale, fake, divergent, or shape-only completion rejects. Child payload/resume authority binds the exact parent, manifest, request, base, predecessor, and reservation.
