...
#### Planned continuation v2 — not implemented

Option A(a) / D is approved for a future v2, but it is **planned and not implemented**. It creates no current runtime promise and does not change the readable v1 narrow `continuation.review.required_fixes` path above. Current v1 post-PR continuation behavior is unchanged. The complete future contract and ownership boundary are in `CONTINUATION-SCOPE-DESIGN.md`; the packaged future-only schema is in `assets/skills/feature/SCHEMA.md`.

V2 will admit only a pre-PR parent whose status is exactly `blocked`, with no PR and no active post-PR state. One child will own the full remaining plan. Its closed `carry_forward` will bind exactly `scope`, parent plan ref/hash, `start_commit`, exact accepted entries (`id`, `attempts`, evidence ref/hash, review ref/hash, `reviewed_commit`, and `merge_commit`), and remaining slice IDs. Accepted IDs plus remaining IDs must exactly equal the plan's ordered IDs; remaining slices inherit no parent authority. Accepted merges must form the no-extra-commit B0MR first-parent chain from the target base to `start_commit`. Complete parent B0MR panels at that head are optional evidence and are never inherited; every child gets a fresh final panel.

Origin-base handling is ordered: unchanged may proceed; an origin base that contains `start_commit` returns `rebaseline-required`; another moved base returns `stale-parent-base-moved`; unavailable observation fails closed. Candidate build, publication, and adoption each recheck the complete contract.

The planned exclusive claim hashes closed parent identity as recursively lexicographic canonical UTF-8 JSON with no whitespace or trailing newline. Its literal ref is `refs/opencode/continuations/<64hex>`. A closed canonical claim blob includes the full `child_branch_ref` and contains no self ref/hash/OID. One no-replace `update-ref` transaction creates the claim ref to that blob and the child branch to `start_commit`, both with zero old OIDs; only exact same-child replay succeeds, and the worktree comes afterward. Crashes before/after commit yield neither/both refs, half-state or different-child collisions fail closed, and the permanent claim tombstone survives cleanup while allowing exact same-child recovery.

Delivery stays split: B1.2 builds/rechecks without creating resources; B1.3 creates only the claim, branch, and then worktree without semantic publication; B1.4 atomically publishes the child and full plan, adopts the accepted prefix, initializes remaining slices with no inherited authority, leaves panels fresh, and activates execution. B1.1 is documentation/tests only and changes no production behavior or current catalog claim.
