...
- Oversized routing requires the exact same-attempt APPROVE work-decomposer review already bound in accepted step authority. The deterministic content-addressed checkpoint manifest binds that review plus exact plan/admission authority, and the parent terminal result is closed to `blocked`, reason `oversized-plan-checkpoint-routing-required`, and exactly its `checkpoint_routing` artifact. Start each entry with `factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json`; the command validates the canonical GitHub origin, fetches exact `refs/heads/main` into a checked temporary ref without trusting local or remote-tracking main, binds that commit, and re-fetches immediately before launch while cleaning the temporary ref. Both reservation refs transition together through `reserved`, `launching`, `launched`, or fail-closed `unknown`. Existing-child adoption requires a matching pre-existing `launching` or `launched` reservation; a fresh reservation is never wrapped around an existing child. Checkpoint N additionally reads both predecessor reservation refs, requires the same exact claim/binding as `run.checkpoint` in state exactly `launched`, proves the full child pipeline, and confirms the canonical GitHub merge commit is on canonical remote main. Local-only, stale, forged, reserved/launching/unknown, wrong-repository, divergent, or shape-only completion rejects.
