...
- Oversized routing requires the exact same-attempt APPROVE work-decomposer review already bound in accepted step authority. The deterministic content-addressed checkpoint manifest binds that review plus exact plan/admission authority, and the parent terminal result is closed to `blocked`, reason `oversized-plan-checkpoint-routing-required`, and exactly its `checkpoint_routing` artifact. Start each entry with `factory checkpoint-start <parent-run-id> <checkpoint-id> --run-id <child-run-id> --json`; parent `main`, omitted base, and `refs/heads/main` normalize only to canonical remote `refs/heads/main`, while every other ref rejects. The command fetches that ref without trusting local or remote-tracking main, atomically reserves the child/route/branch, creates or verifies the child worktree at the exact fetched commit, and re-fetches immediately before launch. Step 0 must reuse those resources and persist top-level `base_ref`/`base_commit` exactly equal to `run.checkpoint`. Resume re-fetches canonical remote main: reserved/launching children fail stale before implementation when it advances, launched children retain their original identity across descendant advances, and divergence routes explicitly without rebasing. Completed predecessors still require both reservation refs at the same exact launched claim plus full pipeline and canonical merged-PR authority.
