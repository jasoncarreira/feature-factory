...
Delivery stays split: B1.2 builds/rechecks without creating resources; B1.3 creates only the claim, branch, and then worktree without semantic publication; B1.4 atomically publishes the child and full plan, adopts accepted slices without rerunning them, initializes remaining slices with no inherited authority, leaves panels fresh, and activates remaining work by normal dependency readiness. B1.1 is documentation/tests only and changes no production behavior or current catalog claim.
