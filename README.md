...
OpenCode compaction does not transfer authority to the model-authored summary. The plugin exact-matches the child session's original checked prompt, revalidates the active claim, accepted plan and input hashes, slice identity, and descendant worktree identity, then re-injects the same checked context as plugin-owned system text. If a callback has already returned without publishing its authenticated closure, the claim remains unresolved by default. After independently inspecting, committing, and verifying the exact work, a trusted local operator may run `feature-factory factory slice-dispatch-reconcile <run-id> <slice-id> <attempt> --authorize-callback-returned-without-closure --json`. The command accepts no caller-provided refs, hashes, token, output, session ID, call ID, or completion SHA. It derives a new clean descendant HEAD and create-publishes `checked-slice-builder-dispatch-reconciliation` in the claim's exclusive `.closed.json` completion slot. A callback closure and operator reconciliation are mutually exclusive, and either record remains subject to focused tests, independent review, ownership checks, and exact reviewed-HEAD equality.
...
feature-factory factory cancel <run-id> --json
feature-factory factory slice-dispatch-reconcile <run-id> <slice-id> <attempt> --authorize-callback-returned-without-closure --json
...
- The durable authority catalog contains 196 variants: 195 production-covered rows, including all 48 generic amendment rows, plus the sole future-only `final-plan-descriptor` row.
- `factory repair` remains only for persisted legacy incidents and the disjoint blocked, previously attempted, or branch-only consumer classes. Those retained incidents still receive one bounded merged-sibling repair per run. A new generic-eligible legacy report rejects before publication and names the exact `factory amendment ... report` command. Generic tombstones and manifests never fall back to legacy, while existing legacy records retain their original schema and legal transitions through terminal state.
