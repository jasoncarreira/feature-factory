...
- Slice review and merged rows require complete current bindings plus append-only attempt history; legacy slice review/merged rows reject without mutation. Validator rows bind `{report_hash, review_hash, reviewed_head_sha}` and security rows bind `{review_hash, reviewed_head_sha}`; both panel rows are successor-bound or both legacy. Checked panel publication binds both atomically to the same clean integration HEAD, and exact checked replays may upgrade only unchanged dual-panel rows. Partial successor tuples reject, successor merged rows stay immutable, and legacy completed runs remain read-only.
...
- Newly produced and seeded plans carry a closed root `integration_gate.required_commands` list of 1-32 ordered structured `{program,args}` entries, never shell text. Programs are trimmed, 1-255 UTF-8 bytes, and free of NUL/control characters; each command has at most 64 args, each at most 4096 UTF-8 bytes and without NUL; the encoded list is at most 64 KiB. Exact `{program:"npm",args:["run","check"]}` occurs once and last. `factory slices-seed --from plan/slices.json` resolves only exact `$RUN/plan/slices.json`, requires a regular non-symlink file with fatal UTF-8 decoding and an exact slice projection, and re-observes its bytes at commit. Work-decomposer acceptance reuses the closed step `acceptance` shape to hash-bind exact `plan/slices.json` plus its review; test-verifier and schema-v2 consumers rehash both. Legacy v1 plans/accepted steps remain readable, but new plans and schema-v2 carry-forward require current bindings and preserve exact plan bytes/hash/order.
- Every current plan with `integration_gate` requires closed `plan.delivery_envelope` schema v1. It has exactly one ordered delivery unit per slice, globally unique lowercase kebab-case family, obligation, and artifact IDs, and exact artifact index/text bindings to that slice's `test_plan`. Admission is active: a unit with multiple invariant families and at least six obligations, or a dependency graph deeper than four waves, routes to `checkpoint`; every other valid current envelope routes to `admit`. Legacy plans without `integration_gate` remain readable and may omit the envelope, but omission grants no B4 authority.
- Every slice-review attempt under a delivery envelope requires a complete closed `invariant_family_ledger`: exactly one disposition per current family, mapped through a current obligation to an artifact, with the enclosing reviewed commit and zero omitted or extra families. Before review, the orchestrator runs each selected exact artifact command only through `feature-factory factory artifact-execute <run-id> <slice-id> <artifact-id> --json`. Under the run lock the command create-publishes a nonce-bound claim before spawn; the claim binds run, slice, attempt, accepted plan hash, reviewed HEAD, artifact ID, and exact parsed program/argv. Completion create-publishes the receipt and closes the claim with the exact result and receipt hash. Active or unknown claims, unclaimed/pre-existing receipts, wrong nonces, stale/replayed bytes, caller-claimed pass results, failed/skipped dispositions, or unresolved findings cannot grant APPROVE authority or trigger duplicate execution.
- Oversized routing uses exact kind `delivery-checkpoint-routing-manifest`. The shared continuation constructor used by both `continueFactory` and `buildContinuation` excludes routing parents and every `delivery-checkpoint-child` before configuration resolution or any dry-run, reservation, branch/worktree, publication, or launch side effect. `factory checkpoint-start` fetches canonical remote `refs/heads/main`, atomically reserves child/route/branch, claim-binds a registered worktree, and factory-publishes the closed child identity before launch. Reserved-to-launching CAS verifies both reservation refs, fetched remote ref, and exact base branch beside a clean-worktree check; launcher return alone never grants `launched`. Ordinary resume/recovery launch paths require the dual-ref reservation state to be exactly `launched`: `reserved` points only to `checkpoint-start` exact replay, while `launching` and `unknown` return recovery-required reconciliation without spawning. Every decoded payload whose dual reservation refs have reached `launched` must also reobserve the exact child `run.json` checkpoint binding and top-level base, branch, and reservation-worktree identity, including the initial non-resume command. Checkpoint test acceptance and every panel/Gate 3/fence/PR/predecessor consumer require the full passing checked execution tuple whose ordered commands exactly equal the selected manifest request. Exact content-addressed blocked routing parents intentionally retain empty `run.slices`; status/list/validate verify their terminal, plan, review, admission, and manifest authority without reapplying ordinary dependency-depth projection checks, while malformed lookalikes remain invalid. Base, branch, and reservation worktree identity are reobserved throughout. Local Git configuration is trusted, but mutable local refs never represent remote authority. After the final canonical PR merges, `factory checkpoint-close <parent-run-id> --json` create-publishes the cataloged final closure after fresh remote-main ancestry and full route authority. Replay returns that exact stored closure, including its historical remote-main commit and closure time, only while both its merge and stored remote-main commits remain ancestors of freshly observed canonical remote main; descendant advancement is valid and divergence rejects.
- Generic checkpoint-child `run.json` mutation requires exact `launched` parent, manifest, dual-ref claim, binding, base, branch, and registered-worktree authority at admission and immediately before replacement. Reserved permits only `checkpoint-start` exact replay; launching, unknown, missing, and cross-bound authority cannot terminalize, recover, heartbeat, steer, advance gates/steps/slices/panels, record cost/environment/provenance, or reconcile fence/PR state. Bootstrap/start/final-close are dedicated internal transitions. Cleanup always preserves checkpoint lineage and reservation refs, even with `--force`; no checked destructive route exists. Process cancellation and heartbeat stop remain allowed because they update only owned sidecars and never `run.json`.
...
Before either schema-v1 seed publication or schema-v2 allocation, both routes contend on `refs/opencode/continuation-targets/<sha256-child-run-id>`. Its canonical create-only blob binds route schema, crash-stable `created_at`, and the hash of the complete continuation authority. Only exact replay may reuse it; a different schema or authority fails before child publication, claim, branch, or worktree mutation. Payload, resume, and publication consumers recheck it.
...
A V1 continuation run is appropriate only when the parent is still based on an acceptable target and one validated review's `required_fixes` accurately bounds the remaining remediation.

| Pattern | Use when | Entry point | Reused authority |
|---|---|---|---|
| **V1 continuation run** | One validated review's `required_fixes` accurately bounds the remaining remediation, or the parent is post-PR. | Unflagged `factory continue` (and `--new-pr` post-PR) | The validated parent relationship and, when durably accepted and unchanged, its planning artifacts. |
...
Use unflagged `feature-factory factory continue <blocked-run-id> --review <review-ref> --run-id <new-run-id>` when its validated review is the complete remaining work order. That schema-v1 route is intentionally narrow: continuation decomposition covers `continuation.review.required_fixes`, not every concern elsewhere. Use explicit `--carry-forward` only when its stricter schema-v2 full-plan eligibility holds. If multiple findings are not represented by that accepted full plan, or a required fix contradicts the accepted brief, start a recovery run rather than silently dropping them.
