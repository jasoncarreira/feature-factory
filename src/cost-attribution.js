...
export function publicCostAttributionSummary(runOrAttribution) {
  ...
  const summary = {
    schema_version: COST_ATTRIBUTION_SCHEMA_VERSION,
    updated_at: attribution?.updated_at ?? null,
    status: attribution?.status || totals.status,
    ...
    missing: Array.isArray(totals.missing) ? [...totals.missing] : [],
  };
  ...
  if (totals.cost_currency !== undefined) summary.cost_currency = totals.cost_currency;
  return summary;
}
  ...
  for (const entry of entries) {
    ...
    for (const field of NUMERIC_FIELDS) {
      ...
      if (COST_NUMERIC_FIELDS.includes(field)) {
        const currency = nonEmptyString(entry.cost_currency);
        if (currency) {
