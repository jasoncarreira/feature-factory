...
Commands:
  ...
  factory repair <run-id> <reported|repairing|review|merged|blocked> [--owner-slice ID --consumer-slice ID --defect-path PATH --evidence-ref REF] [--attempts N] [--review-ref REF] [--merge-commit SHA] [--reason TEXT]
...
async function repairStatus(args) {
  ...
  if (input.status === "review") input.review_ref = requiredOption(opts.reviewRef, "--review-ref", "factory repair review");
  if (input.status === "merged") input.merge_commit = requiredOption(opts.mergeCommit, "--merge-commit", "factory repair merged");
